...
namespace LMM
{
  ...
  namespace Parameters
  {
    ...
    void
    Problem::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Problem");
      {
        prm.declare_entry("Problem",
                          "IsotonicContraction",
                          Patterns::Selection(
                            "IsotonicContraction|BicepsBrachii"),
    ...
    void
    Problem::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Problem");
      {
    ...
    void
    IsotonicContraction::declare_parameters(ParameterHandler & /*prm*/)
    {}

    void
    IsotonicContraction::parse_parameters(ParameterHandler & /*prm*/)
    {}
    ...
    void
    BicepsBrachii::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Biceps Brachii geometry");
      {
        prm.declare_entry("Axial length",
                          "250",
                          Patterns::Double(0),
                          "Axial length of the muscle");

        prm.declare_entry("Radius insertion and origin",
                          "5",
                          Patterns::Double(0),
                          "Insertion and origin radius");

        prm.declare_entry("Radius midpoint",
                          "7.5",
                          Patterns::Double(0),
        ...
        prm.declare_entry("Elements along axis",
                          "32",
                          Patterns::Integer(2),
                          "Number of elements along the muscle axis");

        prm.declare_entry("Radial refinements",
                          "4",
                          Patterns::Integer(0),
    ...
    void
    BicepsBrachii::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Biceps Brachii geometry");
      {
    ...
    void
    NeurologicalSignal::declare_parameters(ParameterHandler &prm)
    {
    ...
    void
    NeurologicalSignal::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Neurological signal");
      {
    ...
    Time::declare_parameters(ParameterHandler &prm)
    {
      ...
      {
        prm.declare_entry("End time", "3", Patterns::Double(0), "End time");

        prm.declare_entry("End ramp time",
                          "1",
                          Patterns::Double(0),
                          "Force ramp end time");
  ...
  {
  public:
    BodyForce(const double rho, const Tensor<1, dim> direction);
    virtual ~BodyForce()
    {}
  ...
  template <int dim>
  BodyForce<dim>::BodyForce(const double rho, const Tensor<1, dim> direction)
    : Function<dim>(dim)
    , rho(rho)
    , g(9.81)
    , M(direction)
  {
    Assert(M.norm() == 1.0,
           ExcMessage("Direction vector is not a unit vector"));
  }
  ...
  {
  public:
    Traction(const double force, const double area);
    virtual ~Traction()
    {}
  ...
  template <int dim>
  Traction<dim>::Traction(const double force, const double area)
    : Function<dim>(dim)
    , t(force / area)
  {}
  ...
  template <int dim>
  inline void
  Traction<dim>::vector_value(const Point<dim> & /*p*/,
                              Vector<double> &values) const
  {
    Assert(values.size() == dim, ExcDimensionMismatch(values.size(), dim));
  ...
  template <int dim>
  void
  Traction<dim>::vector_value_list(
    const std::vector<Point<dim>> &points,
    std::vector<Vector<double>> &  value_list) const
  {
    Assert(value_list.size() == points.size(),
  ...
  template <int dim>
  inline Tensor<2, dim>
    get_deformation_gradient(std::vector<Tensor<1, dim>> &grad)
  {
  ...
  inline SymmetricTensor<2, dim>
    get_small_strain(std::vector<Tensor<1, dim>> &grad)
  {
    Assert(grad.size() == dim, ExcInternalError());
  ...
  const double MuscleMatrix::mu =
    MuscleMatrix::E / (2.0 * (1.0 + MuscleMatrix::nu));
  const double MuscleMatrix::lambda =
    2.0 * MuscleMatrix::mu * MuscleMatrix::nu / (1.0 - 2.0 * MuscleMatrix::nu);
...
#define T0 6280.0 * convert_gf_per_cm2_to_N_per_m2
  ...
  struct MuscleFibre
  {
    ...
    void
    update_alpha(const double u, const double dt);

    void
    update_state(const SymmetricTensor<2, dim> &strain_tensor, const double dt);
    ...
    get_m_s() const;
    double
    get_beta(const double dt) const;
    double
    get_gamma(const double dt) const;
  ...
  template <int dim>
  void
  MuscleFibre<dim>::update_alpha(const double u, const double dt)
  {
    ...
    if (u == 1.0)
      alpha =
        (alpha_t1 * tau_r * tau_f + dt * tau_f) / (tau_r * tau_f + dt * tau_f);
    else if (u == 0)
      alpha = (alpha_t1 * tau_r * tau_f + dt * alpha_min * tau_r) /
              (tau_r * tau_f + dt * tau_r);
    else
      {
  ...
  template <int dim>
  double
  MuscleFibre<dim>::get_m_s(void) const
  {
  ...
  template <int dim>
  double
  MuscleFibre<dim>::get_f_c_L(void) const
  {
  ...
  template <int dim>
  double
  MuscleFibre<dim>::get_m_c_V(void) const
  {
  ...
  template <int dim>
  double
  MuscleFibre<dim>::get_c_c_V(void) const
  {
    if (epsilon_c_dot < -5.0)
      return 0.0;
    else if (epsilon_c_dot <= 3.0)
  ...
  template <int dim>
  double
  MuscleFibre<dim>::get_beta(const double dt) const
  {
  ...
  template <int dim>
  double
  MuscleFibre<dim>::get_gamma(const double dt) const
  {
    return get_f_c_L() * alpha *
           (get_m_c_V() * epsilon_c_t1 / dt - get_c_c_V());
  }

  template <int dim>
  void
  MuscleFibre<dim>::update_state(const SymmetricTensor<2, dim> &strain_tensor,
                                 const double                   dt)
  {
  ...
  private:
    ...
    setup_muscle_fibres();
    double
    get_neural_signal(const double time);
    void
    update_fibre_activation(const double time);
    void
    update_fibre_state();
    void
    setup_system();
    ...
    solve();
    void
    output_results(const unsigned int timestep, const double time) const;
    ...
    get_stiffness_tensor(const unsigned int cell,
                         const unsigned int q_point_cell) const;
    SymmetricTensor<2, dim>
    get_rhs_tensor(const unsigned int cell,
                   const unsigned int q_point_cell) const;
  };
  ...
  LinearMuscleModelProblem<dim>::LinearMuscleModelProblem(
    ...
    , body_force(
        (parameters.problem == "BicepsBrachii" &&
         parameters.include_gravity == true) ?
          BodyForce<dim>(0.375 * 1000.0,
                         Tensor<1, dim>(
                           {0, -1, 0})) : // (reduced) Density and direction
          BodyForce<dim>(0.0, Tensor<1, dim>({0, 0, 1})))
    , traction(
        parameters.problem == "BicepsBrachii" ?
          Traction<dim>(parameters.axial_force, // Force, area
                        M_PI * std::pow(parameters.radius_insertion_origin *
                                          parameters.scale,
                                        2.0)) :
          Traction<dim>(4.9 * convert_gf_to_N, // Force; Conversion of gf to N,
                        (2.0 * parameters.half_length_y) *
                          (2.0 * parameters.half_length_z))) // Area
  {
    Assert(dim == 3, ExcNotImplemented());
  }
  ...
  struct BicepsGeometry
  {
    ...
    // the muscle
    Point<dim>
    profile(const Point<dim> &pt_0) const
    {
      Assert(pt_0[0] > -1e-6,
    ...
    Point<dim>
    operator()(const Point<dim> &pt) const
    {
      return profile(pt);
    }
    ...
    Tensor<1, dim>
    direction(const Point<dim> &pt_scaled, const double &grid_scale) const
    {
      ...
      Assert(inv_profile(pt_eps_p).distance(pt_0_eps_p) < tol,
             ExcInternalError());
      Assert(inv_profile(pt_eps_m).distance(pt_0_eps_m) < tol,
             ExcInternalError());
  ...
  private:
    ...
    double
    get_radial_scaling_factor(const double &x) const
    {
      ...
      const double wave_func =
        y_shift + std::cos(phase_shift + 2.0 * M_PI * lgth_frac);
      Assert(wave_func >= 0.0, ExcInternalError());
      return std::sqrt(amplitude * wave_func);
    }

    Point<dim>
    inv_profile(const Point<dim> &pt) const
    {
      Assert(pt[0] > -1e-6,
             ExcMessage("All points must have x-coordinate > 0"));
      ...
      return Point<dim>(pt[0],
                        trans_inv_scale * pt[1],
                        trans_inv_scale * pt[2]);
    }
  };

  template <int dim>
  void
  LinearMuscleModelProblem<dim>::make_grid()
  {
    Assert(dim == 3, ExcNotImplemented());
    ...
    if (parameters.problem == "IsotonicContraction")
                ...
                if (cell->face(face)->at_boundary() == true)
                  {
                    if (cell->face(face)->center()[0] ==
                        -parameters.half_length_x) // -X oriented face
                      cell->face(face)->set_boundary_id(
                        parameters.bid_CC_dirichlet_symm_X); // Dirichlet
                    else if (cell->face(face)->center()[0] ==
                             parameters.half_length_x) // +X oriented face
                      cell->face(face)->set_boundary_id(
                        parameters.bid_CC_neumann); // Neumann
                    else if (std::abs(cell->face(face)->center()[2]) ==
                             parameters.half_length_z) // -Z/+Z oriented face
                      cell->face(face)->set_boundary_id(
                        parameters.bid_CC_dirichlet_symm_Z); // Dirichlet
                  }
              }
          }
    ...
    else if (parameters.problem == "BicepsBrachii")
      {
        ...
        struct GridRotate
        {
          ...
          {
            static const Tensor<2, dim> rot_mat =
              Physics::Transformations::Rotations::rotation_matrix_3d(
                Point<dim>(0, 1, 0), M_PI / 2.0);
        ...
        GridTools::transform(
          BicepsGeometry<dim>(parameters.axial_length,
                              parameters.radius_insertion_origin,
                ...
                if (cell->face(face)->at_boundary() == true)
                  {
                    ...
                    if (std::abs(cell->face(face)->center()[0]) <
                        tol) // -X oriented face
                      cell->face(face)->set_boundary_id(
                        parameters.bid_BB_dirichlet_X); // Dirichlet
                    else if (std::abs(cell->face(face)->center()[0] -
                                      parameters.axial_length) <
                             tol) // +X oriented face
                      cell->face(face)->set_boundary_id(
                        parameters.bid_BB_neumann); // Neumann
                  }
              }
          }
  ...
  template <int dim>
  void
  LinearMuscleModelProblem<dim>::setup_muscle_fibres()
  {
    fibre_data.clear();
    ...
    if (parameters.problem == "IsotonicContraction")
      {
        ...
        for (unsigned int cell_no = 0; cell_no < triangulation.n_active_cells();
             ++cell_no)
          {
            fibre_data[cell_no].resize(n_q_points_cell);
            for (unsigned int q_point_cell = 0; q_point_cell < n_q_points_cell;
                 ++q_point_cell)
              {
        ...
        for (typename Triangulation<dim>::active_cell_iterator cell =
             ...
             ++cell, ++cell_no)
          {
            ...
            for (unsigned int q_point_cell = 0; q_point_cell < n_q_points_cell;
              ...
              {
                const Point<dim> pt =
                  fe_values.get_quadrature_points()[q_point_cell];
                fibre_data[cell_no][q_point_cell] =
                  MuscleFibre<dim>(bicep_geom.direction(pt, parameters.scale));
              }
          }
      }
    else
  ...
  template <int dim>
  double
  LinearMuscleModelProblem<dim>::get_neural_signal(const double time)
  {
    // Note: 40 times less force generated than Martins2006
    // This is necessary due to the (compliant) linear tissue model
    return (time > parameters.neural_signal_start_time &&
                time < parameters.neural_signal_end_time ?
              1.0 / 40.0 :
              0.0);
  }
  ...
  template <int dim>
  void
  LinearMuscleModelProblem<dim>::update_fibre_activation(const double time)
  {
  ...
  template <int dim>
  void
  LinearMuscleModelProblem<dim>::update_fibre_state()
  {
    const unsigned int n_q_points_cell = qf_cell.size();
    ...
    std::vector<std::vector<Tensor<1, dim>>> u_grads(
      n_q_points_cell, std::vector<Tensor<1, dim>>(dim));
    ...
    for (typename DoFHandler<dim>::active_cell_iterator cell =
         ...
         ++cell, ++cell_no)
      {
        ...
        for (unsigned int q_point_cell = 0; q_point_cell < n_q_points_cell;
          ...
          {
            Assert(
              q_point_cell < fibre_data[cell_no].size(),
              ExcMessage(
                "Trying to access fibre data not stored for this cell and qp index"));

            const SymmetricTensor<2, dim> strain_tensor =
              get_small_strain(u_grads[q_point_cell]);
  ...
  template <int dim>
  void
  LinearMuscleModelProblem<dim>::setup_system()
  {
    ...
    const unsigned int q_point_cell) const
  {
    ...
    Assert(cell < fibre_data.size(),
           ExcMessage(
             "Trying to access fibre data not stored for this cell index"));
    Assert(
      q_point_cell < fibre_data[cell].size(),
      ExcMessage(
        "Trying to access fibre data not stored for this cell and qp index"));
  ...
  template <int dim>
  void
  LinearMuscleModelProblem<dim>::assemble_system(const double time)
  {
    // Reset system
    system_matrix = 0;
    system_rhs    = 0;
    ...
    for (typename DoFHandler<dim>::active_cell_iterator cell =
         ...
         ++cell, ++cell_no)
      {
        ...
        for (unsigned int q_point_cell = 0; q_point_cell < n_q_points_cell;
             ++q_point_cell)
          {
            const SymmetricTensor<4, dim> C =
              get_stiffness_tensor(cell_no, q_point_cell);
            const SymmetricTensor<2, dim> R =
              get_rhs_tensor(cell_no, q_point_cell);
            ...
            for (unsigned int I = 0; I < dofs_per_cell; ++I)
              {
                ...
                cell_rhs(I) += fe_values.shape_value(I, q_point_cell) *
                               body_force_values[q_point_cell](component_I) *
                               fe_values.JxW(q_point_cell);

                for (unsigned int k = 0; k < dim; ++k)
                  cell_rhs(I) += (fe_values.shape_grad(I, q_point_cell)[k] *
                                  R[component_I][k]) *
                                 fe_values.JxW(q_point_cell);
              }
          }
            ...
            if (cell->face(face)->at_boundary() == true &&
                    ...
                    parameters.bid_BB_neumann)))
              {
                ...
                traction.vector_value_list(
                  fe_face_values.get_quadrature_points(), traction_values);

                // Scale applied traction according to time
                const double ramp =
                  (time <= t_ramp_end ? time / t_ramp_end : 1.0);
                Assert(ramp >= 0.0 && ramp <= 1.0,
                       ExcMessage("Invalid force ramp"));
                for (unsigned int q_point_face = 0;
                     q_point_face < n_q_points_face;
                     ++q_point_face)
  ...
  template <int dim>
  void
  LinearMuscleModelProblem<dim>::apply_boundary_conditions()
  {
    ...
    if (parameters.problem == "IsotonicContraction")
      {
        ...
        // Fixed point on -X face
        {
          ...
          for (typename DoFHandler<dim>::active_cell_iterator
               ...
               ++cell)
            {
              for (unsigned int face = 0;
                   ...
                   ++face)
                {
                  // We know that the fixed point is on the -X Dirichlet
                  // boundary
                  if (cell->face(face)->at_boundary() == true &&
                      cell->face(face)->boundary_id() ==
                        parameters.bid_CC_dirichlet_symm_X)
                    {
                      for (unsigned int face_vertex_index = 0;
                           face_vertex_index <
                           GeometryInfo<dim>::vertices_per_face;
                           ++face_vertex_index)
                        {
                          if (cell->face(face)
                                ->vertex(face_vertex_index)
                                .distance(fixed_point) < 1e-6)
                            {
                              found_point_of_interest = true;
                              for (unsigned int index_component = 0;
                                   index_component < dim;
                                   ++index_component)
                                fixed_dof_indices.push_back(
                                  cell->face(face)->vertex_dof_index(
                                    face_vertex_index, index_component));
                            }

                          if (found_point_of_interest == true)
                            break;
                        }
                    }
                  if (found_point_of_interest == true)
          ...
          Assert(found_point_of_interest == true,
                 ExcMessage("Didn't find point of interest"));
    ...
    else if (parameters.problem == "BicepsBrachii")
      {
        ...
        {
          const Point<dim> roller_point(
            parameters.axial_length * parameters.scale, 0.0, 0.0);
          std::vector<types::global_dof_index> fixed_dof_indices;
          bool                                 found_point_of_interest = false;
                        ...
                        parameters.bid_BB_neumann)
                    {
                      for (unsigned int face_vertex_index = 0;
                           ...
                           ++face_vertex_index)
                        {
                          if (cell->face(face)
                                ->vertex(face_vertex_index)
                                .distance(roller_point) < 1e-6)
                            {
                              found_point_of_interest = true;
                              for (unsigned int index_component = 1;
                                   index_component < dim;
                                   ++index_component)
                                fixed_dof_indices.push_back(
                                  cell->face(face)->vertex_dof_index(
  ...
  template <int dim>
  void
  LinearMuscleModelProblem<dim>::solve()
  {
  ...
  template <int dim>
  void
  LinearMuscleModelProblem<dim>::output_results(const unsigned int timestep,
                                                const double       time) const
  {
    ...
    // Visual output: FEM data
    {
      ...
      std::vector<std::vector<double>> output_values(
        n_results, std::vector<double>(n_data_points));
      ...
      for (typename DoFHandler<dim>::active_cell_iterator cell =
           ...
           ++cell, ++cell_no)
        {
          ...
          for (unsigned int q_point_cell = 0; q_point_cell < n_q_points_cell;
               ++q_point_cell, ++fibre_no)
            {
              const MuscleFibre<dim> &fibre = fibre_data[cell_no][q_point_cell];
              output_points[fibre_no] =
                fe_values.get_quadrature_points()[q_point_cell]; // Position
              for (unsigned int d = 0; d < dim; ++d)
                output_displacements[fibre_no][d] =
                  u_values[q_point_cell][d]; // Displacement
              // Direction (spatial configuration)
              output_directions[fibre_no] =
                get_deformation_gradient(u_grads[q_point_cell]) * fibre.get_M();
      ...
      // FIBRE POSITION
      output << "POINTS " << n_data_points << " float" << std::endl;
      ...
      output << "\nPOINT_DATA " << n_data_points << std::endl << std::endl;

      // FIBRE DISPLACEMENTS
      output << "VECTORS displacement float" << std::endl;
      for (unsigned int i = 0; i < n_data_points; ++i)
        {
          for (unsigned int j = 0; j < dim; ++j)
            {
      ...
      // FIBRE DIRECTIONS
      output << "VECTORS direction float" << std::endl;
      for (unsigned int i = 0; i < n_data_points; ++i)
        {
          for (unsigned int j = 0; j < dim; ++j)
            {
      ...
      for (unsigned int v = 0; v < n_results; ++v)
        {
          output << "SCALARS  " << results_fibre_names[v] << "  float 1"
                 << std::endl
    ...
    // Output X-displacement at measured point
    {
          ...
          for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell;
               ++face)
            {
              ...
              if (cell->face(face)->at_boundary() == true &&
                ...
                {
                  for (unsigned int face_vertex_index = 0;
                       face_vertex_index < GeometryInfo<dim>::vertices_per_face;
                       ++face_vertex_index)
                    {
                      if (cell->face(face)
                            ->vertex(face_vertex_index)
                            .distance(meas_pt) < 1e-6)
                        {
                          found_point_of_interest = true;
                          dof_of_interest = cell->face(face)->vertex_dof_index(
                            face_vertex_index, index_of_interest);
                        }
      ...
      Assert(found_point_of_interest == true,
             ExcMessage("Didn't find point of interest"));
      Assert(dof_of_interest != numbers::invalid_dof_index,
             ExcMessage("Didn't find DoF of interest"));
      Assert(dof_of_interest < dof_handler.n_dofs(),
             ExcMessage("DoF index out of range"));
      ...
      output << time << "," << solution[dof_of_interest] * 1e3 << std::endl;
    }
  }
  ...
  template <int dim>
  void
  LinearMuscleModelProblem<dim>::run()
  {
    make_grid();
    setup_system();
    setup_muscle_fibres();
