...
#  error DEAL_II_WITH_PETSC or DEAL_II_WITH_TRILINOS required
  ...
  // formulas:
  // - Longitude (x-y plane angle): atan2(y, x) * 180 / π
  // - Latitude (z-radial angle): atan2(z, sqrt(x² + y²)) * 180 / π
  double
  ...
  public:
    FlowRoutingMatrix(
      ...
      : x_with_ghosts(locally_owned_water_dofs,
                      ...
                      mpi_communicator)
      , local_flow_routing(local_flow_routing)
    {
      ...
      for (auto &[src, dst] : this->local_flow_routing)
        {
      ...
      const auto it =
        std::remove_if(this->local_flow_routing.begin(),
                       this->local_flow_routing.end(),
      ...
      this->local_flow_routing.erase(it, this->local_flow_routing.end());
      ...
      write_buffer_source_indices.resize(this->local_flow_routing.size());
      write_buffer_indices.resize(this->local_flow_routing.size());
      write_buffer_values.resize(this->local_flow_routing.size());
      unsigned int index = 0;
      for (const auto &[src, dst] : this->local_flow_routing)
        {
  ...
  public:
      ...
      : local_flow_routing(local_flow_routing)
    {
      ...
      AssertDimension(this->local_flow_routing.size(),
      ...
      for (auto &src_dst : this->local_flow_routing)
  ...
  public:
      ...
      , flow_routing_preconditioner(flow_routing_preconditioner)
    {
      ...
      Assert((Utilities::MPI::n_mpi_processes(mpi_communicator) > 1) ||
               (this->local_flow_routing.size() == 0),
             ExcInternalError());
