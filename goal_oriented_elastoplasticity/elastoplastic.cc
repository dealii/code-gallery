...
namespace ElastoPlastic
{
                        ...
                        Triangulation<3, 3> &      result)
  {
    ...
    for (Triangulation<2, 2>::cell_iterator cell = input.begin_active();
         cell != input.end();
         ++cell)
      {
  ...
  namespace Evaluation
  {
    template <int dim>
    double
    get_von_Mises_stress(const SymmetricTensor<2, dim> &stress)
    {
      //      if (dim == 2)
      //      {
      ...
      // plane stress
      //      const double von_Mises_stress = std::sqrt(
      ...
      const double von_Mises_stress =
        std::sqrt(1.5) * (deviator(stress)).norm();
      // -----------------------------------------------
    ...
    template <int dim>
    PointValuesEvaluation<dim>::PointValuesEvaluation(
      const Point<dim> &evaluation_point)
    ...
    template <int dim>
    void
    PointValuesEvaluation<dim>::compute(const DoFHandler<dim> &dof_handler,
                                        const Vector<double> & solution,
                                        Vector<double> &       point_values)
    {
                ...
                if (cell->vertex(vertex).distance(evaluation_point) <
                    cell->diameter() * 1e-8)
                  {
                    for (unsigned int id = 0; id != dofs_per_vertex; ++id)
                      {
                        point_values[id] =
                          solution(cell->vertex_dof_index(vertex, id));
                      }
  ...
  ConstitutiveLaw<dim>::ConstitutiveLaw(double E,
    ...
    : //--------------------
      // Plane stress
    //    kappa (((E*(1+2*nu)) / (std::pow((1+nu),2))) / (3 * (1 - 2 * (nu /
    //    (1+nu))))), mu (((E*(1+2*nu)) / (std::pow((1+nu),2))) / (2 * (1 + (nu
    //    / (1+nu))))),
    //--------------------
    // 3d and plane strain
    kappa(E / (3 * (1 - 2 * nu)))
    ...
    SymmetricTensor<4, dim> &      stress_strain_tensor) const
  {
    ...
    //    deviator(stress_tensor); const double deviator_stress_tensor_norm =
    //    deviator_stress_tensor.norm();
    const double von_Mises_stress =
      Evaluation::get_von_Mises_stress(stress_tensor);
  ...
  template <int dim>
  bool
  ConstitutiveLaw<dim>::get_grad_stress_strain_tensor(
    const SymmetricTensor<2, dim> &    strain_tensor,
    const std::vector<Tensor<2, dim>> &point_hessian,
    Tensor<5, dim> &                   stress_strain_tensor_grad) const
  {
    ...
    if (von_Mises_stress > sigma_0)
      {
        const SymmetricTensor<2, dim> deviator_strain_tensor =
          deviator(strain_tensor);
        const double deviator_strain_tensor_norm =
          deviator_strain_tensor.norm();
        const double multiplier =
          -(1 - gamma) * sigma_0 /
          (2 * mu * std::pow(deviator_strain_tensor_norm, 3));
        ...
        for (unsigned int i = 0; i != dim; ++i)
          for (unsigned int m = 0; m != dim; ++m)
            for (unsigned int n = 0; n != dim; ++n)
              {
                multiplier_vector(i) +=
                  deviator_strain_tensor[m][n] *
                  (0.5 * (point_hessian[m][n][i] + point_hessian[n][m][i]) +
                   (m == n && dim == 2 ?
                      -1 / dim *
                        (point_hessian[0][0][i] + point_hessian[1][1][i]) :
                      0) +
                   (m == n && dim == 3 ?
                      -1 / dim *
                        (point_hessian[0][0][i] + point_hessian[1][1][i] +
          ...
          for (unsigned int j = 0; j != dim; ++j)
            for (unsigned int k = 0; k != dim; ++k)
              for (unsigned int l = 0; l != dim; ++l)
                for (unsigned int m = 0; m != dim; ++m)
                  {
                    stress_strain_tensor_grad[i][j][k][l][m] =
                      1 / VM_factor * multiplier *
  ...
  namespace EquationData
  {
    ...
    */
                  ...
                  Vector<double>   &values) const
    {
      ...
      values(0) =  fac * y * ( (6*length-3*x)*x +
    (2+Poissons_ratio)*(y*y-depth*depth/4) ); values(1) = -fac* (
    3*Poissons_ratio*y*y*(length-x) + 0.25*(4+5*Poissons_ratio)*depth*depth*x +
    (3*length-x)*x*x );
    ...
    */
    ...
    private:
      const double present_time, end_time, pressure, height;
    };

    template <int dim>
    IncrementalBoundaryForce<dim>::IncrementalBoundaryForce(
      const double present_time,
      const double end_time)
      : Function<dim>(dim)
      , present_time(present_time)
    ...
    template <int dim>
    void
    IncrementalBoundaryForce<dim>::vector_value_list(
    ...
    private:
      const double present_time, end_time;
    };
  ...
  namespace DualFunctional
  {
    ...
    template <int dim>
    void
    PointValuesEvaluation<dim>::assemble_rhs(
    ...
    template <int dim>
    PointXDerivativesEvaluation<dim>::PointXDerivativesEvaluation(
      const Point<dim> &evaluation_point)
      : evaluation_point(evaluation_point)
    {}
    ...
    template <int dim>
    void
    PointXDerivativesEvaluation<dim>::assemble_rhs(
      const DoFHandler<dim> & /*dof_handler*/,
      const Vector<double> & /*solution*/,
      const ConstitutiveLaw<dim> & /*constitutive_law*/,
      const DoFHandler<dim> &dof_handler_dual,
      Vector<double> &       rhs_dual) const
    {
      ...
      FEValues<dim>      fe_values(dof_handler_dual.get_fe(),
                              quadrature,
      ...
      typename DoFHandler<dim>::active_cell_iterator cell = dof_handler_dual
                                                              .begin_active(),
                                                     endc =
    ...
    template <int dim>
    MeanDisplacementFace<dim>::MeanDisplacementFace(
      const unsigned int      face_id,
      const std::vector<bool> comp_mask)
      : face_id(face_id)
      , comp_mask(comp_mask)
    ...
    template <int dim>
    void
    MeanDisplacementFace<dim>::assemble_rhs(
      const DoFHandler<dim> & /*dof_handler*/,
      const Vector<double> & /*solution*/,
      const ConstitutiveLaw<dim> & /*constitutive_law*/,
      const DoFHandler<dim> &dof_handler_dual,
      Vector<double> &       rhs_dual) const
    {
      ...
      const QGauss<dim - 1> face_quadrature(
        dof_handler_dual.get_fe().tensor_degree() + 1);
      FEFaceValues<dim> fe_face_values(dof_handler_dual.get_fe(),
                                       face_quadrature,
    ...
    template <int dim>
    MeanStressFace<dim>::MeanStressFace(
      const unsigned int                            face_id,
      const std::vector<std::vector<unsigned int>> &comp_stress)
      : face_id(face_id)
    ...
    template <int dim>
    void
    MeanStressFace<dim>::assemble_rhs(
      const DoFHandler<dim> &     dof_handler,
      const Vector<double> &      solution,
      const ConstitutiveLaw<dim> &constitutive_law,
      const DoFHandler<dim> &     dof_handler_dual,
      Vector<double> &            rhs_dual) const
    {
      ...
      FEFaceValues<dim> fe_face_values(dof_handler.get_fe(),
                                       face_quadrature,
                                       update_gradients);
      FEFaceValues<dim> fe_face_values_dual(dof_handler_dual.get_fe(),
                                            face_quadrature,
                                            update_gradients |
                                              update_JxW_values);

      const unsigned int dofs_per_cell_dual =
        dof_handler_dual.get_fe().dofs_per_cell;
      const unsigned int n_face_q_points = face_quadrature.size();
      ...
      typename DoFHandler<dim>::active_cell_iterator
        cell_dual = dof_handler_dual.begin_active(),
        endc_dual = dof_handler_dual.end(), cell = dof_handler.begin_active();
                  ...
                  for (unsigned int q_point = 0; q_point < n_face_q_points;
                       ++q_point)
                    {
                      ...
                      constitutive_law.get_stress_strain_tensor(
                        strain_tensor[q_point], stress_strain_tensor);
    ...
    template <int dim>
    MeanStressDomain<dim>::MeanStressDomain(
      const std::string &                           base_mesh,
      const std::vector<std::vector<unsigned int>> &comp_stress)
      : base_mesh(base_mesh)
      , comp_stress(comp_stress)
    ...
    template <int dim>
    void
    MeanStressDomain<dim>::assemble_rhs(
      const DoFHandler<dim> &     dof_handler,
      const Vector<double> &      solution,
      const ConstitutiveLaw<dim> &constitutive_law,
      const DoFHandler<dim> &     dof_handler_dual,
      Vector<double> &            rhs_dual) const
    {
      ...
      const double height = 200e-3, thickness_flange = 10e-3;
      ...
      FEValues<dim> fe_values(dof_handler.get_fe(),
                              quadrature_formula,
                              update_gradients);
      FEValues<dim> fe_values_dual(dof_handler_dual.get_fe(),
                                   quadrature_formula,
      ...
      for (; cell_dual != endc_dual; ++cell_dual, ++cell)
        {
          const double y = cell->center()[1], z = cell->center()[2];
          // top domain: height/2 - thickness_flange <= y <= height/2
          //             0 <= z <= 2 * thickness_flange
          // bottom domain: -height/2 <= y <= -height/2 + thickness_flange
          //             0 <= z <= 2 * thickness_flange
                  ...
                  for (unsigned int i = 0; i < dofs_per_cell_dual; ++i)
                    {
                      const SymmetricTensor<2, dim> stress_phi_i =
                        stress_strain_tensor *
                        fe_values_dual[displacement].symmetric_gradient(
                          i, q_point);
    ...
    template <int dim>
    MeanStrainEnergyFace<dim>::MeanStrainEnergyFace(
      const unsigned int   face_id,
      const Function<dim> &lambda_function,
      const Function<dim> &mu_function)
      : face_id(face_id)
    ...
    template <int dim>
    void
    MeanStrainEnergyFace<dim>::assemble_rhs_nonlinear(
      const DoFHandler<dim> &primal_dof_handler,
      const Vector<double> & primal_solution,
      const DoFHandler<dim> &dof_handler,
      Vector<double> &       rhs) const
    {
      // Assemble right hand side of the dual problem when the quantity of
      // interest is a nonlinear functinoal. In this case, the QoI should be
      // linearized which depends on the solution of the primal problem. The
      // extracter of the linearized QoI functional is the gradient of the the
      // original QoI functional with the primal solution values.
      ...
      FEFaceValues<dim> primal_fe_face_values(primal_dof_handler.get_fe(),
                                              face_quadrature,
      ...
      const unsigned int dofs_per_vertex =
        primal_dof_handler.get_fe().dofs_per_vertex;
      const unsigned int n_face_q_points = face_quadrature.size();
               ...
               ++face)
            {
              if (cell->face(face)->at_boundary() &&
                  cell->face(face)->boundary_id() == face_id)
                {
                  ...
                  primal_fe_face_values.get_function_gradients(
                    primal_solution, primal_solution_gradients);

                  primal_fe_face_values.get_function_hessians(
                    primal_solution, primal_solution_hessians);

                  lambda_function->value_list(
                    primal_fe_face_values.get_quadrature_points(),
                    lambda_values);
                  mu_function->value_list(
                    primal_fe_face_values.get_quadrature_points(), mu_values);
                      ...
                      for (unsigned int m = 0; m < dofs_per_cell; ++m)
                        {
                          const unsigned int component_m =
                            dof_handler.get_fe()
                              .system_to_component_index(m)
                              .first;

                          for (unsigned int i = 0; i != dofs_per_vertex; ++i)
                            {
                              for (unsigned int j = 0; j != dofs_per_vertex;
                                   ++j)
                                {
                                  cell_rhs(m) +=
                                    fe_face_values.shape_value(m, q_point) *
                                    (lambda_values[q_point] *
                                       (primal_solution_hessians[q_point][i][i]
                                                                [component_m] *
                                          primal_solution_gradients[q_point][j]
                                                                   [j] +
                                        primal_solution_gradients[q_point][i]
                                                                 [i] *
                                          primal_solution_hessians
                                            [q_point][j][j][component_m]) +
                                     mu_values[q_point] *
                                       (2 *
                                          primal_solution_hessians
                                            [q_point][j][i][component_m] *
                                          primal_solution_gradients[q_point][j]
                                                                   [i] +
                                        primal_solution_hessians[q_point][i][j]
                                                                [component_m] *
                                          primal_solution_gradients[q_point][j]
                                                                   [i] +
                                        primal_solution_gradients[q_point][i]
  ...
  public:
    ...
    void
    compute_error_DWR(Vector<float> &estimated_error_per_cell);
  ...
  DualSolver<dim>::setup_system()
  {
    ...
    std::cout << "    Number of degrees of freedom in dual problem:  "
              << dof_handler_dual.n_dofs() << std::endl;
    ...
    constraints_dirichlet_and_hanging_nodes_dual.condense(
      sparsity_pattern_dual);
  ...
  template <int dim>
  void
  DualSolver<dim>::compute_dirichlet_constraints()
  {
    constraints_dirichlet_and_hanging_nodes_dual.clear();
    constraints_dirichlet_and_hanging_nodes_dual.merge(
      constraints_hanging_nodes_dual);
  ...
  DualSolver<dim>::assemble_matrix()
  {
    ...
    FEValues<dim> fe_values_dual(fe_dual,
                                 quadrature_formula,
  ...
  {
    dual_functional->assemble_rhs(dof_handler,
                                  solution,
                                  constitutive_law,
                                  dof_handler_dual,
                                  system_rhs_dual);
  ...
  template <int dim>
  void
  DualSolver<dim>::compute_error_DWR(Vector<float> &estimated_error_per_cell)
  {
    Assert(estimated_error_per_cell.size() ==
             triangulation->n_global_active_cells(),
           ExcDimensionMismatch(estimated_error_per_cell.size(),
                                triangulation->n_global_active_cells()));
    ...
    // estimate the error
    FEValues<dim> fe_values(fe_dual,
                            quadrature_formula,
    ...
    FEFaceValues<dim> fe_face_values_cell(fe_dual,
      ...
      fe_face_values_neighbor(fe_dual,
                              face_quadrature_formula,
                              update_values | update_gradients |
                                update_JxW_values | update_normal_vectors);
    FESubfaceValues<dim> fe_subface_values_cell(fe_dual,
                                                face_quadrature_formula,
                                                update_gradients);
          ...
          for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
            {
              constitutive_law.get_linearized_stress_strain_tensors(
                ...
                stress_strain_tensor_linearized,
                stress_strain_tensor);
              constitutive_law.get_grad_stress_strain_tensor(
                strain_tensor[q_point],
                      ...
                      for (unsigned int k = 0; k != dim; ++k)
                        {
                          for (unsigned int l = 0; l != dim; ++l)
                            {
                              error_indicators_vector[present_cell](i) +=
                                (stress_strain_tensor[i][j][k][l] * 0.5 *
                                   (cell_hessians[q_point][k][l][j] +
                                    cell_hessians[q_point][l][k][j]) +
                                 stress_strain_tensor_grad[i][j][k][l][j] *
                                   strain_tensor[q_point][k][l]) *
                                dual_weights_cell_values[q_point](i) *
                                fe_values.JxW(q_point);
                            }
                        }
              ...
              if (cell->face(face_no)->has_children() == false)
                {
                  ...
                  fe_face_values_neighbor.get_function_gradients(
                    primal_solution, neighbor_grads);
                      ...
                      for (unsigned int i = 0; i != dim; ++i)
                        {
                          for (unsigned int j = 0; j != dim; ++j)
                            {
                              q_cell_strain_tensor[i][j] =
                                0.5 * (cell_grads[q_point][i][j] +
                                       cell_grads[q_point][j][i]);
                              q_neighbor_strain_tensor[i][j] =
                                0.5 * (neighbor_grads[q_point][i][j] +
                                       neighbor_grads[q_point][j][i]);
                            }
                        }

                      constitutive_law.get_stress_strain_tensor(
                        q_cell_strain_tensor, cell_stress_strain_tensor);
                      constitutive_law.get_stress_strain_tensor(
                        q_neighbor_strain_tensor,
                              ...
                              for (unsigned int k = 0; k != dim; ++k)
                                {
                                  for (unsigned int l = 0; l != dim; ++l)
                                    {
                                      jump_residual[q_point](i) +=
                                        (cell_stress_strain_tensor[i][j][k][l] *
                                           q_cell_strain_tensor[k][l] -
                                         neighbor_stress_strain_tensor[i][j][k]
                                                                      [l] *
                                           q_neighbor_strain_tensor[k][l]) *
                                        fe_face_values_cell.normal_vector(
                                          q_point)[j];
                                    }
                                }
                            }
                  ...
                  fe_face_values_cell.get_function_values(
                    dual_weights, dual_weights_face_values);
                  ...
                  for (unsigned int i = 0; i != dim; ++i)
                    {
                      Assert(face_integrals[cell->face(face_no)](i) == -1e20,
                             ExcInternalError());
                      face_integrals[cell->face(face_no)](i) =
                        face_integral_vector(i);
                    }
              ...
              else
                {
                  ...
                  for (unsigned int subface_no = 0;
                       ...
                       ++subface_no)
                    {
                                  ...
                                  for (unsigned int k = 0; k != dim; ++k)
                                    {
                                      for (unsigned int l = 0; l != dim; ++l)
                                        {
                                          jump_residual[q_point](i) +=
                                            (-cell_stress_strain_tensor[i][j][k]
                                                                       [l] *
                                               q_cell_strain_tensor[k][l] +
                                             neighbor_stress_strain_tensor[i][j]
                                                                          [k]
                                                                          [l] *
                                               q_neighbor_strain_tensor[k][l]) *
                                            fe_face_values_neighbor
                                              .normal_vector(q_point)[j];
                                        }
                                    }
                                }
                            }
                      ...
                      for (unsigned int i = 0; i != dim; ++i)
                        {
                          face_integrals[neighbor_child->face(
                            neighbor_neighbor)](i) = face_integral_vector(i);
                        }
                    }
                  ...
                  for (unsigned int subface_no = 0;
                       ...
                       ++subface_no)
                    {
                      ...
                      for (unsigned int i = 0; i != dim; ++i)
                        {
                          Assert(face_integrals[face->child(subface_no)](i) !=
                                   -1e20,
                                 ExcInternalError());
      ...
      if (cell->is_locally_owned())
        {
          ...
          estimated_error_per_cell(present_cell) =
            error_indicators_vector[present_cell].l2_norm();
        }
  }
  ...
  private:
    ...
    compute_error();
    void
    compute_error_residual(const TrilinosWrappers::MPI::Vector &tmp_solution);
    void
    refine_grid();
    void
    ...
    // to indicate the MPI universe this program runs on, and then two numbers
    ...
    std::string  output_dir;
    TableHandler table_results, table_results_2, table_results_3;
  ...
  ElastoPlasticProblem<dim>::declare_parameters(ParameterHandler &prm)
  {
    ...
    prm.declare_entry("isotropic hardening parameter",
                      "0.",
                      Patterns::Double(),
  ...
  template <int dim>
  ElastoPlasticProblem<dim>::ElastoPlasticProblem(const ParameterHandler &prm)
    ...
    , show_stresses(prm.get_bool("show stresses"))
  {
    ...
    if (strat == "kelly_error")
      ...
      error_estimation_strategy =
        ErrorEstimationStrategy::weighted_residual_error;
    else
      AssertThrow(false, ExcNotImplemented());
    ...
    if (base_mesh == "Timoshenko beam")
      {
        ...
        const double length = .48, depth = .12;

        const Point<dim> point_1(0, -depth / 2), point_2(length, depth / 2);
        ...
        // 100 to the null boundaries (upper and lower edges) where we do not
        // need to take care of them 5 to the natural boundaries (right edge)
    ...
    else if (base_mesh == "Thick_tube_internal_pressure")
      {
        ...
        // determining collapse load-carrying capacity of structure made of
        // elasto-plastic material, J. Cent. South Univ. (2014) 21: 398-404
        AssertThrow(dim == 2, ExcNotImplemented());
             ...
             |_          \
              ...
              0 \         |
                 |________|
                     3
        */
        // 0 - inner boundary  - natural boundary condition - impose the
        // traction force 1 - outer boundary  - free boundary - we do not need
        // to take care of them 2 - left boundary   - essential boundary
        // condition - constrained to move along the x direction 3 - bottom
        // boundary - essential boundary condition - constrained to move along
        // the y direction
      ...
      {
        // Example 2 from the paper: Zhong Z., .... A new numerical method for
        // determining collapse load-carrying capacity of structure made of
        // elasto-plastic material, J. Cent. South Univ. (2014) 21: 398-404
        AssertThrow(dim == 3, ExcNotImplemented());
        ...
        const double inner_radius = 0.05, outer_radius = 0.1, height = 0.18,
                     thickness = 0.004;
        //                   thickness = 0.01;

        Triangulation<dim_2d> triangulation_1, triangulation_2,
          triangulation_2d;
        ...
        const double eps = 1e-7 * inner_radius;
        {
          ...
          GridGenerator::quarter_hyper_shell(
            triangulation_1, center_2d, inner_radius, outer_radius, 2);
          ...
          for (; cell != endc; ++cell)
            {
              for (unsigned int f = 0; f < GeometryInfo<dim_2d>::faces_per_cell;
                   ++f)
                if (cell->face(f)->at_boundary() &&
                    cell->face(f)->center()(0) > eps &&
                    ...
                    if (point.norm() > (inner_radius + eps))
                      {
                        for (unsigned int v = 0;
                             v < GeometryInfo<dim_2d>::vertices_per_face;
                             ++v)
                          {
            ...
            for (; cell != endc; ++cell)
              {
                for (unsigned int f = 0;
                     f < GeometryInfo<dim_2d>::faces_per_cell;
                     ++f)
                  {
                    if (cell->face(f)->at_boundary())
                      {
        ...
        //                                           2, thickness,
        //                                           triangulation);
        extrude_triangulation(triangulation_2d, 2, thickness, triangulation);
        ...
        const CylindricalManifold<dim> inner_boundary_description(inner_radius,
                                                                  2);
    ...
    else if (base_mesh == "Cantiliver_beam_3d")
      {
        ...
        const double length = .7, width = 80e-3, height = 200e-3,
                     thickness_web = 10e-3, thickness_flange = 10e-3;

        Triangulation<dim_2d> triangulation_b, triangulation_t, triangulation_l,
          triangulation_r, triangulation_2d;

        const double eps = 1e-7 * width;
        // Make the triangulation_b, a rectangular at the bottom of rectangular
        // tube
        {
          ...
          GridGenerator::subdivided_hyper_rectangle(triangulation_b,
                                                    repetitions,
                                                    point1,
                                                    point2);
        }

        // Make the triangulation_t, a rectangular at the top of rectangular
        // tube
        {
          const Point<dim_2d> point1(-width / 2,
          ...
          GridGenerator::subdivided_hyper_rectangle(triangulation_t,
                                                    repetitions,
                                                    point1,
                                                    point2);
        }

        // Make the triangulation_l, a rectangular at the left of rectangular
        // tube
        {
          const Point<dim_2d> point1(-width / 2,
                                     -(height / 2) + thickness_flange),
          ...
          GridGenerator::subdivided_hyper_rectangle(triangulation_l,
                                                    repetitions,
                                                    point1,
                                                    point2);
        }

        // Make the triangulation_r, a rectangular at the right of rectangular
        // tube
        {
          const Point<dim_2d> point1((width / 2) - thickness_web,
                                     -(height / 2) + thickness_flange),
            point2(width / 2, (height / 2) - thickness_flange);
          ...
          GridGenerator::subdivided_hyper_rectangle(triangulation_r,
                                                    repetitions,
                                                    point1,
                                                    point2);
        }
        ...
        // make the triangulation_2d
        {
          ...
          Triangulation<dim_2d> triangulation_bl, triangulation_blr;

          GridGenerator::merge_triangulations(triangulation_b,
                                              triangulation_l,
                                              triangulation_bl);
          GridGenerator::merge_triangulations(triangulation_bl,
                                              triangulation_r,
                                              triangulation_blr);
          GridGenerator::merge_triangulations(triangulation_blr,
                                              triangulation_t,
                                              triangulation_2d);
        }
  ...
  // particular in AffineConstraints<double>::distribute_local_to_global(),
  // using the constraints we have previously computed.
  template <int dim>
  void
  ElastoPlasticProblem<dim>::assemble_newton_system(
    const TrilinosWrappers::MPI::Vector & /*linearization_point*/,
    const TrilinosWrappers::MPI::Vector &delta_linearization_point)
  {
    ...
    const EquationData::IncrementalBoundaryForce<dim> boundary_force(
      ...
      if (cell->is_locally_owned())
        {
          ...
          fe_values[displacement].get_function_symmetric_gradients(
            delta_linearization_point, incremental_strain_tensor);
              ...
              for (unsigned int i = 0; i < dofs_per_cell; ++i)
                {
                  ...
                  cell_rhs(i) +=
                    ((stress_phi_i * incremental_strain_tensor[q_point]) -
                     ((stress_strain_tensor *
                       fe_values[displacement].symmetric_gradient(i, q_point)) *
                      tmp_strain_tensor_qpoint) +
  ...
  template <int dim>
  void
  ElastoPlasticProblem<dim>::compute_nonlinear_residual(
    const TrilinosWrappers::MPI::Vector &linearization_point)
  {
    types::boundary_id traction_surface_id = numbers::invalid_boundary_id;
    if (base_mesh == "Timoshenko beam")
      ...
      if (cell->is_locally_owned())
        {
          ...
          cell->get_dof_indices(local_dof_indices);
          constraints_dirichlet_and_hanging_nodes.distribute_local_to_global(
            cell_rhs, local_dof_indices, newton_rhs_residual);
        }
  ...
  //   non-zero with a meaningless value. These values we have to to set to
  //   zero.
  ...
  ElastoPlasticProblem<dim>::solve_newton_system()
  {
    ...
    //    {
    //      TimerOutput::Scope t(computing_timer, "Solve: setup
    //      preconditioner");
    //
    ...
    //      solver(solver_control); solver.solve(newton_matrix,
    //      distributed_solution,
  ...
  // an elastic problem in very first Newton step on each mesh (or only on the
  ...
  // then setting it back to the correct value in subsequent iterations.
  //
  ...
  ElastoPlasticProblem<dim>::solve_newton()
  {
    ...
    TrilinosWrappers::MPI::Vector distributed_solution(locally_owned_dofs,
                                                       mpi_communicator);
    TrilinosWrappers::MPI::Vector tmp_solution(locally_owned_dofs,
                                               mpi_communicator);

    double residual_norm;
    double previous_residual_norm = -std::numeric_limits<double>::max();

    double disp_norm, previous_disp_norm = 0;

    const double correct_sigma = sigma_0;

    const unsigned int max_newton_iter = 100;

    for (unsigned int newton_step = 1; newton_step <= max_newton_iter;
         ++newton_step)
      {
        if (newton_step == 1 &&
            ((transfer_solution && timestep_no == 1) || !transfer_solution))
            ...
            for (unsigned int i = 0; i < 5; i++)
              {
                ...
                pcout << "      Residual of the system: " << residual_norm
                      << std::endl
        ...
        pcout
          << "    difference of two consecutive incremental displacement l2 norm : "
          << std::abs(disp_norm - previous_disp_norm) << std::endl;
        if (std::abs(disp_norm - previous_disp_norm) < 1e-10 &&
            (residual_norm < 1e-5 ||
             std::abs(residual_norm - previous_residual_norm) < 1e-9))
          {
            pcout << " Convergence by difference of two consecutive solution! "
                  << std::endl;
            break;
          }
  ...
  ElastoPlasticProblem<dim>::compute_error()
  {
    ...
    if (error_estimation_strategy == ErrorEstimationStrategy::kelly_error)
    ...
    else if (error_estimation_strategy ==
             ErrorEstimationStrategy::residual_error)
      {
             ...
             ErrorEstimationStrategy::weighted_residual_error)
      {
        ...
        // the dual function definition (it should be defined previously, e.g.
        // input file)
        if (base_mesh == "Timoshenko beam")
          {
            double length = .48, depth = .12;

            const Point<dim> evaluation_point(length, -depth / 2);

            DualFunctional::PointValuesEvaluation<dim> dual_functional(
              evaluation_point);
        ...
        else if (base_mesh == "Thick_tube_internal_pressure")
          {
            ...
            DualFunctional::MeanStressFace<dim> dual_functional(face_id,
                                                                comp_stress);
        ...
        else if (base_mesh == "Cantiliver_beam_3d")
          {
            ...
            DualFunctional::PointValuesEvaluation<dim>
            dual_functional(evaluation_point);
            */

            // -----------------------------------------------------------
            // Mean stress at the specified domain is of interest.
            // The interest domains are located on the bottom and top of the
            // flanges close to the clamped face, z = 0 top domain: height/2 -
            // thickness_flange <= y <= height/2
            //             0 <= z <= 2 * thickness_flange
            // bottom domain: -height/2 <= y <= -height/2 + thickness_flange
            //             0 <= z <= 2 * thickness_flange
            ...
            DualFunctional::MeanStressDomain<dim> dual_functional(base_mesh,
                                                                  comp_stress);
    ...
    relative_error =
      estimated_error_per_cell.l2_norm() / tmp_solution.l2_norm();
  ...
  template <int dim>
  void
  ElastoPlasticProblem<dim>::compute_error_residual(
    const TrilinosWrappers::MPI::Vector &tmp_solution)
  {
    FEValues<dim> fe_values(fe,
                            quadrature_formula,
                            update_values | update_gradients | update_hessians |
    ...
    FEFaceValues<dim> fe_face_values_cell(fe,
                                          face_quadrature_formula,
                                          update_values |
                                            update_quadrature_points |
                                            update_gradients |
      ...
      fe_face_values_neighbor(fe,
                              ...
                              update_values | update_gradients |
                                update_JxW_values | update_normal_vectors);
    FESubfaceValues<dim> fe_subface_values_cell(fe,
                                                face_quadrature_formula,
                                                update_gradients);
  ...
  ElastoPlasticProblem<dim>::refine_grid()
  {
    ...
    std::vector<std::vector<Vector<double>>> history_stress_field(
      dim, std::vector<Vector<double>>(dim)),
      local_history_stress_values_at_qpoints(dim,
                                             std::vector<Vector<double>>(dim)),
      local_history_stress_fe_values(dim, std::vector<Vector<double>>(dim));


    std::vector<std::vector<Vector<double>>> history_strain_field(
      dim, std::vector<Vector<double>>(dim)),
      ...
      for (unsigned int j = 0; j < dim; j++)
        {
          ...
          local_history_stress_values_at_qpoints[i][j].reinit(
          ...
          local_history_strain_values_at_qpoints[i][j].reinit(
            quadrature_formula.size());
    ...
    FETools::compute_projection_from_quadrature_points_matrix(
      history_fe, quadrature_formula, quadrature_formula, qpoint_to_dof_matrix);
    typename DoFHandler<dim>::active_cell_iterator
      cell = dof_handler.begin_active(),
    ...
    if (refinement_strategy == RefinementStrategy::refine_global)
      ...
      {
        const double refine_fraction_cells = .3, coarsen_fraction_cells = .03;
    ...
    parallel::distributed::SolutionTransfer<dim, TrilinosWrappers::MPI::Vector>
      incremental_displacement_transfer(dof_handler);
    if (transfer_solution)
      incremental_displacement_transfer.prepare_for_coarsening_and_refinement(
        incremental_displacement);

    SolutionTransfer<dim, Vector<double>> history_stress_field_transfer0(
      history_dof_handler),
      history_stress_field_transfer1(history_dof_handler),
      history_stress_field_transfer2(history_dof_handler);
    history_stress_field_transfer0.prepare_for_coarsening_and_refinement(
      history_stress_field[0]);
    if (dim > 1)
      {
        history_stress_field_transfer1.prepare_for_coarsening_and_refinement(
          history_stress_field[1]);
      }
    if (dim == 3)
      {
        history_stress_field_transfer2.prepare_for_coarsening_and_refinement(
          history_stress_field[2]);
      }

    SolutionTransfer<dim, Vector<double>> history_strain_field_transfer0(
      history_dof_handler),
      history_strain_field_transfer1(history_dof_handler),
      history_strain_field_transfer2(history_dof_handler);
    history_strain_field_transfer0.prepare_for_coarsening_and_refinement(
      history_strain_field[0]);
    if (dim > 1)
      {
        history_strain_field_transfer1.prepare_for_coarsening_and_refinement(
          history_strain_field[1]);
      }
    if (dim == 3)
      {
        history_strain_field_transfer2.prepare_for_coarsening_and_refinement(
          history_strain_field[2]);
      }
    ...
    if (transfer_solution)
      {
        TrilinosWrappers::MPI::Vector distributed_incremental_displacement(
          locally_owned_dofs, mpi_communicator);
        //        distributed_incremental_displacement =
        //        incremental_displacement;
        incremental_displacement_transfer.interpolate(
          distributed_incremental_displacement);
    ...
    history_dof_handler.distribute_dofs(history_fe);
    // stress
    std::vector<std::vector<Vector<double>>> distributed_history_stress_field(
      dim, std::vector<Vector<double>>(dim));
    for (unsigned int i = 0; i < dim; i++)
      for (unsigned int j = 0; j < dim; j++)
        {
          distributed_history_stress_field[i][j].reinit(
            history_dof_handler.n_dofs());
        }

    history_stress_field_transfer0.interpolate(
      history_stress_field[0], distributed_history_stress_field[0]);
    if (dim > 1)
      {
        history_stress_field_transfer1.interpolate(
          history_stress_field[1], distributed_history_stress_field[1]);
      }
    if (dim == 3)
      {
        history_stress_field_transfer2.interpolate(
          history_stress_field[2], distributed_history_stress_field[2]);
      }

    history_stress_field = distributed_history_stress_field;

    // strain
    std::vector<std::vector<Vector<double>>> distributed_history_strain_field(
      dim, std::vector<Vector<double>>(dim));
    for (unsigned int i = 0; i < dim; i++)
      for (unsigned int j = 0; j < dim; j++)
        {
          distributed_history_strain_field[i][j].reinit(
            history_dof_handler.n_dofs());
        }

    history_strain_field_transfer0.interpolate(
      history_strain_field[0], distributed_history_strain_field[0]);
    if (dim > 1)
      {
        history_strain_field_transfer1.interpolate(
          history_strain_field[1], distributed_history_strain_field[1]);
      }
    if (dim == 3)
      {
        history_strain_field_transfer2.interpolate(
          history_strain_field[2], distributed_history_strain_field[2]);
      }
    ...
    FETools::compute_interpolation_to_quadrature_points_matrix(
      history_fe, quadrature_formula, dof_to_qpoint_matrix);
    cell    = dof_handler.begin_active();
    endc    = dof_handler.end();
  ...
  template <int dim>
  void
  ElastoPlasticProblem<dim>::setup_quadrature_point_history()
  {
  ...
  template <int dim>
  void
  ElastoPlasticProblem<dim>::update_quadrature_point_history()
  {
      ...
      if (cell->is_locally_owned())
        {
          ...
          fe_values.reinit(cell);
          fe_values[displacement].get_function_symmetric_gradients(
            incremental_displacement, incremental_strain_tensor);
  ...
  template <int dim>
  void
  ElastoPlasticProblem<dim>::move_mesh(
  ...
  ElastoPlasticProblem<dim>::output_results(const std::string &filename_base)
  {
      ...
      if (cell->is_locally_owned())
        {
          ...
          for (unsigned int q = 0; q < quadrature_formula.size(); ++q)
            {
              ...
              for (unsigned int i = 0; i < dim; i++)
                for (unsigned int j = i; j < dim; j++)
                  {
                    local_history_stress_values_at_qpoints[i][j](q) =
                      stress_at_qpoint[i][j];
                  }

              local_VM_stress_values_at_qpoints(q) =
                Evaluation::get_von_Mises_stress(stress_at_qpoint);
            }
          ...
          dg_cell->set_dof_values(local_VM_stress_fe_values, VM_stress_field);
        }
    ...
    AssertThrow(dof_handler_1.n_dofs() == triangulation.n_vertices(),
                ExcDimensionMismatch(dof_handler_1.n_dofs(),
                                     triangulation.n_vertices()));

    std::vector<std::vector<Vector<double>>> history_stress_on_vertices(
      dim, std::vector<Vector<double>>(dim));
    for (unsigned int i = 0; i < dim; i++)
      for (unsigned int j = 0; j < dim; j++)
        {
          ...
          for (unsigned int v = 0; v < GeometryInfo<dim>::vertices_per_cell;
               ++v)
            {
              types::global_dof_index dof_1_vertex =
                cell_1->vertex_dof_index(v, 0);

              // begin check
              //            Point<dim> point1, point2;
              //            point1 = cell_1->vertex(v);
              //            point2 = dg_cell->vertex(v);
              //            AssertThrow(point1.distance(point2) <
              //            cell->diameter()*1e-8, ExcInternalError());
              // end check

              counter_on_vertices(dof_1_vertex) += 1;

              VM_stress_on_vertices(dof_1_vertex) +=
                local_VM_stress_fe_values(v);
    ...
    if (show_stresses)
      {
          ...
          if (this_mpi_process == 0)
            {
              ...
              std::ofstream pvtu_master_output(
                (output_dir + filename_base_stress + ".pvtu").c_str());
              data_out.write_pvtu_record(pvtu_master_output, filenames);

              std::ofstream visit_master_output(
                (output_dir + filename_base_stress + ".visit").c_str());
              data_out.write_pvtu_record(visit_master_output, filenames);
            }
        }

        {
          ...
          data_out.add_data_vector(history_stress_on_vertices[0][0],
                                   "stress_xx_averaged");
          data_out.add_data_vector(history_stress_on_vertices[1][1],
                                   "stress_yy_averaged");
          data_out.add_data_vector(history_stress_on_vertices[0][1],
                                   "stress_xy_averaged");
          data_out.add_data_vector(VM_stress_on_vertices,
                                   "Von_Mises_stress_averaged");

          if (dim == 3)
            {
              data_out.add_data_vector(history_stress_on_vertices[0][2],
                                       "stress_xz_averaged");
              data_out.add_data_vector(history_stress_on_vertices[1][2],
                                       "stress_yz_averaged");
              data_out.add_data_vector(history_stress_on_vertices[2][2],
                                       "stress_zz_averaged");
            }

          data_out.build_patches();

          const std::string filename_base_stress =
            ("averaged-stress-" + filename_base);
    ...
    if (base_mesh == "Timoshenko beam")
      {
        ...
        EquationData::IncrementalBoundaryValues<dim>
          incremental_boundary_values(present_time, end_time);
        incremental_boundary_values.vector_value(intersted_point,
                                                 vertex_exact_displacement);
      ...
      {
        const double pressure(0.6 * 2.4e8), inner_radius(.1);
        //      const double pressure (1.94e8),
        //                   inner_radius (.1);


        // Plane stress
        //      const double mu (((e_modulus*(1+2*nu)) / (std::pow((1+nu),2))) /
        //      (2 * (1 + (nu / (1+nu)))));
        // 3d and plane strain
        ...
        table_results.add_value("pressure/sigma_0",
                                (pressure * present_time / end_time) / sigma_0);
        table_results.add_value("4*mu*u_A/(sigma_0*a)",
                                4 * mu * disp_A(0) / (sigma_0 * inner_radius));

        // Compute stresses in the POLAR coordinates, 1- save it on Gauss
        // points, 2- extrapolate them to nodes and taking their avarages (nodal
        // avaraging)
        AssertThrow(dim == 2, ExcNotImplemented());

        // we define a rotation matrix to be able to transform the stress
        // from the Cartesian coordinate to the polar coordinate
        Tensor<2, dim> rotation_matrix; // [cos sin; -sin cos]    , sigma_r =
                                        // rot * sigma * rot^T
        ...
        std::vector<std::vector<Vector<double>>> history_stress_field(
          dim, std::vector<Vector<double>>(dim)),
          local_history_stress_values_at_qpoints(
            dim, std::vector<Vector<double>>(dim)),
          local_history_stress_fe_values(dim, std::vector<Vector<double>>(dim));
        for (unsigned int i = 0; i < dim; i++)
          for (unsigned int j = 0; j < dim; j++)
            {
              ...
              local_history_stress_values_at_qpoints[i][j].reinit(
                quadrature_formula.size());
              local_history_stress_fe_values[i][j].reinit(
                history_fe.dofs_per_cell);
            }
              ...
              for (unsigned int q = 0; q < quadrature_formula.size(); ++q)
                {
                  ...
                  // transform the stress from the Cartesian coordinate to the
                  // polar coordinate
                  ...
                  stress_at_qpoint = rotation_matrix * stress_at_qpoint *
        ...
        for (unsigned int id = 0; id < dof_handler_1.n_dofs(); ++id)
          {
            for (unsigned int i = 0; i < dim; i++)
              for (unsigned int j = 0; j < dim; j++)
                {
                  history_stress_on_vertices[i][j](id) /=
                    counter_on_vertices(id);
                }
          }
...

        {
          ...
          const std::string filename_base_stress =
            ("averaged-stress-polar-" + filename_base);
        ...
        if (std::abs((present_time / end_time) * (pressure / sigma_0) - 0.6) <
          ...
          {
            // table_results_2: presenting the stress_rr and stress_tt on the
            // nodes of bottom edge
            ...
            typename DoFHandler<dim>::active_cell_iterator
              cell = dof_handler.begin_active(),
              endc = dof_handler.end(), cell_1 = dof_handler_1.begin_active();
                          ...
                          for (unsigned int v = 0;
                               ...
                               ++v)
                            {
                              ...
                              if (!vertices_found[dof_1_vertex])
                                {
                                  const Point<dim> vertex_coordinate =
                                    cell_1->face(face)->vertex(v);

                                  table_results_2.add_value(
                                    "x coordinate", vertex_coordinate[0]);
                                  table_results_2.add_value(
                                    "stress_rr",
                                    history_stress_on_vertices[0][0](
                                      dof_1_vertex));
                                  table_results_2.add_value(
                                    "stress_tt",
                                    history_stress_on_vertices[1][1](
                                      dof_1_vertex));
                                  table_results_2.add_value(
                                    "pressure/sigma_0",
                                    (pressure * present_time / end_time) /
                                      sigma_0);
            ...
            // table_results_3: presenting the mean stress_rr of the nodes on
            // the inner radius
            const unsigned int face_id_2 = 0;

            Tensor<2, dim> stress_node, mean_stress_polar;
            mean_stress_polar = 0;
            ...
            table_results_3.add_value("pressure/sigma_0",
                                      (pressure * present_time / end_time) /
                                        sigma_0);
            table_results_3.add_value("Cells",
                                      triangulation.n_global_active_cells());
            table_results_3.add_value("DoFs", dof_handler.n_dofs());
            table_results_3.add_value("radius", inner_radius);
            table_results_3.add_value("mean stress_rr",
                                      mean_stress_polar[0][0]);
            table_results_3.add_value("mean stress_tt",
                                      mean_stress_polar[1][1]);
          }
      }
    else if (base_mesh == "Perforated_strip_tension")
      {
        const double imposed_displacement(0.00055), inner_radius(0.05);
        ...
        // table_results: Demonstrates the result of displacement at the top
        // left corner versus imposed tension
        /*
        {
          ...
          table_results.add_value("time step", timestep_no);
          table_results.add_value("Cells",
        triangulation.n_global_active_cells()); table_results.add_value("DoFs",
        dof_handler.n_dofs()); table_results.add_value("4*mu*u_C/(sigma_0*r)",
        4*mu*disp_C(1)/(sigma_0*inner_radius));
        }
        */
        ...
        // table_results_2: Demonstrate the stress_yy on the nodes of bottom
        // edge
                          ...
                          cell->face(face)->boundary_id() == face_id)
                        {
                          ...
                          for (unsigned int v = 0;
                               ...
                               ++v)
                            {
                              ...
                              if (!vertices_found[dof_1_vertex] &&
                                  std::abs(vertex_coordinate[2]) < 1.e-8)
                                {
                                  table_results_2.add_value(
                                    "x", vertex_coordinate[0]);
                                  table_results_2.add_value(
                                    "x/r", vertex_coordinate[0] / inner_radius);
                                  table_results_2.add_value(
                                    "stress_xx/sigma_0",
                                    history_stress_on_vertices[0][0](
                                      dof_1_vertex) /
                                  ...
                                  table_results_2.add_value(
                                    ...
                                    history_stress_on_vertices[1][1](
                                      ...
                                      sigma_0);
                                  table_results_2.add_value(
                                    "stress_yy_av/sigma_0",
                                    stress_yy_av / sigma_0);
                                  table_results_2.add_value(
                                    "Imposed u_y",
                                    (imposed_displacement * present_time /
                                     end_time));
        ...
        //  on the bottom edge versus epsilon_yy on the bottom left corner
        {
          ...
          // we need to work just with one element
          {
            ...
            Vector<double> local_strain_yy_values_at_qpoints(
              quadrature_formula.size()),
                  ...
                  for (unsigned int v = 0;
                    ...
                    if (std::fabs(cell->vertex(v)[0] - point_A[0]) < 1e-6 &&
                        ...
                        std::fabs(cell->vertex(v)[2] - point_A[2]) < 1e-6)
                      {
                        ...
                        // Then loop over the quadrature points of this cell:
                        for (unsigned int q = 0; q < quadrature_formula.size();
                             ++q)
                          {
                            strain_at_qpoint =
                              local_quadrature_points_history[q].old_strain;

                            local_strain_yy_values_at_qpoints(q) =
                              strain_at_qpoint[1][1];
                          }

                        qpoint_to_dof_matrix.vmult(
                          local_strain_yy_fe_values,
          ...
          table_results_3.add_value("Cells",
                                    triangulation.n_global_active_cells());
          table_results_3.add_value("DoFs", dof_handler.n_dofs());
          table_results_3.add_value("Imposed u_y",
                                    (imposed_displacement * present_time /
                                     end_time));
          table_results_3.add_value("mean_tensile_stress/sigma_0",
                                    stress_yy_av / sigma_0);
          table_results_3.add_value("E*strain_yy-A/sigma_0",
                                    e_modulus * strain_yy_A / sigma_0);
        }
      ...
      {
        const double pressure(6e6), length(.7), height(200e-3);
        ...
        // front point, Point A
        {
          ...
          Evaluation::PointValuesEvaluation<dim> point_values_evaluation(
            point_A);
          ...
          table_results.add_value("Cells",
                                  triangulation.n_global_active_cells());
          table_results.add_value("DoFs", dof_handler.n_dofs());
          table_results.add_value("pressure",
                                  pressure * present_time / end_time);
  ...
  ElastoPlasticProblem<dim>::run()
  {
    ...
    const SphericalManifold<dim> inner_boundary_description(center,
    inner_radius); triangulation.set_manifold (0, inner_boundary_description);

    const SphericalManifold<dim> outer_boundary_description(center,
    outer_radius); triangulation.set_manifold (1, outer_boundary_description);
    */
    // ----------------------------------------------------------------
    ...
    while (present_time < end_time)
      {
            ...
            if ((timestep_no > 1) && (current_refinement_cycle > 0) &&
                (relative_error >= max_relative_error))
              {
                pcout << "The relative error, " << relative_error
                      ...
                      << max_relative_error
                      << ", but we move to the next increment.\n";
        ...
        */
        ...
        output_results(
          (std::string("solution-") + Utilities::int_to_string(timestep_no, 4))
            .c_str());
  ...
  try
    {
      ...
      if (argc != 2)
        {
          std::cerr << "*** Call this program as <./elastoplastic input.prm>"
                    << std::endl;
          return 1;
        }
