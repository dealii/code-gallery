...
namespace ViscoElasStripHole
{
  ...
  namespace Parameters
  {
    ...
    Geometry::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Geometry");
      {
        ...
        prm.declare_entry("Hole diameter",
                          "20.0",
                          Patterns::Double(0.0),
                          "Hole diameter");
        prm.declare_entry(
    ...
    Geometry::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Geometry");
      {
        ...
        n_repetitions_xy =
          prm.get_integer("Number of subdivisions in cross-section");
    ...
    Materials::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material properties");
      {
        ...
        prm.declare_entry("Viscous shear modulus",
                          "80.194e6",
                          Patterns::Double(0.0),
                          "Viscous shear modulus");
        prm.declare_entry("Viscous relaxation time",
                          "2.0",
                          Patterns::Double(0.0),
    ...
    LinearSolver::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Linear solver");
      {
        prm.declare_entry(
          ...
          "cg",
          Patterns::Selection(
            SolverSelector<LA::MPI::Vector>::get_solver_names()),
  ...
  public:
                                ...
                                const double & /*J_tilde*/)
    {
      ...
      const SymmetricTensor<2, dim> C_bar =
        std::pow(det_F, -2.0 / dim) * Physics::Elasticity::Kinematics::C(F);
      // Linder2011 eq 54
      // Assumes first-oder backward Euler time discretisation
      Q_n_t = (1.0 / (1.0 + time.get_delta_t() / tau_v)) *
              (Q_t1 + (time.get_delta_t() / tau_v) * invert(C_bar));
    }
    void
    update_end_timestep()
    {
  ...
  protected:
    ...
    const Time & time;
    SymmetricTensor<2, dim>
      Q_n_t; // Value of internal variable at this Newton step and timestep
    SymmetricTensor<2, dim>
    ...
    get_tau_bar(const Tensor<2, dim> &F) const
    {
      ...
      return mu_e * b_bar +
             mu_v * symmetrize(F_bar * static_cast<Tensor<2, dim>>(Q_n_t) *
                               transpose(F_bar));
    }
    SymmetricTensor<4, dim>
    ...
    get_c_bar(const Tensor<2, dim> & /*F*/) const
    {
      ...
      return -2.0 * mu_v *
             ((time.get_delta_t() / tau_v) /
              (1.0 + time.get_delta_t() / tau_v)) *
  ...
  public:
    ...
    {}
    void
    setup_lqp(const Parameters::AllParameters &parameters, const Time &time)
    {
      material.reset(
  ...
  private:
    ...
    void
         make_grid();
    void make_2d_quarter_plate_with_hole(
  ...
  Solid<dim>::run()
  {
    ...
    pcout << "\n\n*** Spatial position history for tracked vertices ***"
          << std::endl;
              ...
              if (std::abs(cell->face(face)->center()[0] - 0.0) < tol)
                {
                  cell->face(face)->set_boundary_id(
                    parameters.boundary_id_minus_X);
                }
              else if (std::abs(cell->face(face)->center()[0] -
                                parameters.width / 2.0) < tol)
                {
                  cell->face(face)->set_boundary_id(
                    parameters.boundary_id_plus_X);
                }
              else if (std::abs(cell->face(face)->center()[1] - 0.0) < tol)
                {
                  cell->face(face)->set_boundary_id(
                    parameters.boundary_id_minus_Y);
                }
              else if (std::abs(cell->face(face)->center()[1] -
                                parameters.length / 2.0) < tol)
                {
                  cell->face(face)->set_boundary_id(
                    parameters.boundary_id_plus_Y);
                }
              else
                {
                  for (unsigned int vertex = 0;
                       ...
                       ++vertex)
                    if (std::abs(cell->vertex(vertex).distance(centre) -
                                 parameters.hole_diameter / 2.0) < tol)
                      {
                        cell->face(face)->set_boundary_id(
                          parameters.boundary_id_hole);
                        break;
                      }
                }
  ...
  Solid<3>::make_grid()
  {
          ...
          if (cell->face(face)->at_boundary())
            {
              ...
              if (std::abs(cell->face(face)->center()[0] - 0.0) < tol)
                ...
                {
                  cell->face(face)->set_boundary_id(
                    parameters.boundary_id_minus_Z);
                }
              else if (std::abs(cell->face(face)->center()[2] -
                                parameters.thickness / 2.0) < tol)
                {
                  cell->face(face)->set_boundary_id(
                    parameters.boundary_id_plus_Z);
                }
              else
                {
                  for (unsigned int vertex = 0;
                       ...
                       ++vertex)
                    {
                      ...
                      vertex_proj[2]         = 0.0;
                      if (std::abs(vertex_proj.distance(centre) -
                                   parameters.hole_diameter / 2.0) < tol)
                        {
                          cell->face(face)->set_boundary_id(
                            parameters.boundary_id_hole);
              ...
              for (unsigned int vertex = 0;
                   ...
                   ++vertex)
                {
                  ...
                  vertex_proj[2]         = 0.0;
                  if (std::abs(vertex_proj.distance(centre) -
                               parameters.hole_diameter / 2.0) < 1e-12)
                    {
    ...
    triangulation.set_manifold(parameters.manifold_id_hole,
    ...
    const double       hole_division_fraction)
  {
    ...
    const double internal_width =
      hole_diameter + hole_division_fraction * (width - hole_diameter);
    Triangulation<2> tria_quarter_plate_hole;
    {
      ...
      for (typename Triangulation<2>::active_cell_iterator cell =
             tria_plate_hole.begin_active();
           cell != tria_plate_hole.end();
           ++cell)
        {
      ...
      Assert(cells_to_remove.size() != tria_plate_hole.n_active_cells(),
             ExcInternalError());
      GridGenerator::create_triangulation_with_removed_cells(
        tria_plate_hole, cells_to_remove, tria_quarter_plate_hole);
    }
    ...
    Triangulation<2> tria_cut_plate;
    {
      ...
      std::vector<std::vector<double>> step_sizes;
      {
        ...
        const unsigned int n_subs =
          std::max(1.0, std::ceil(width_remaining / (internal_width / 2.0)));
        ...
        const double sum_half_width = std::accumulate(subdivision_width.begin(),
                                                      subdivision_width.end(),
                                                      0.0);
        Assert(std::abs(sum_half_width - width / 2.0) < 1e-12,
               ExcInternalError());
      }
      {
        ...
        const unsigned int n_subs =
          std::max(1.0, std::ceil(length_remaining / (internal_width / 2.0)));
        Assert(n_subs > 0, ExcInternalError());
        for (unsigned int s = 0; s < n_subs; ++s)
          subdivision_length.push_back(length_remaining / n_subs);
        step_sizes.push_back(subdivision_length);

        const double sum_half_length =
          std::accumulate(subdivision_length.begin(),
                          subdivision_length.end(),
                          0.0);
        Assert(std::abs(sum_half_length - length / 2.0) < 1e-12,
               ExcInternalError());
      }
      ...
      std::set<typename Triangulation<2>::active_cell_iterator> cells_to_remove;
      for (typename Triangulation<2>::active_cell_iterator cell =
             tria_plate.begin_active();
           cell != tria_plate.end();
           ++cell)
        {
          // Remove all cells that are in the first quadrant
          if (cell->center()[0] < internal_width / 2.0 &&
              cell->center()[1] < internal_width / 2.0)
            cells_to_remove.insert(cell);
        }
      Assert(cells_to_remove.size() > 0, ExcInternalError());
      Assert(cells_to_remove.size() != tria_plate.n_active_cells(),
             ExcInternalError());
      GridGenerator::create_triangulation_with_removed_cells(tria_plate,
                                                             cells_to_remove,
                                                             tria_cut_plate);
    }
    ...
    for (typename Triangulation<2>::active_cell_iterator cell =
           tria_2d_not_flat.begin_active();
         cell != tria_2d_not_flat.end();
         ++cell)
      {
        for (unsigned int face = 0; face < GeometryInfo<2>::faces_per_cell;
          ...
          if (cell->face(face)->at_boundary())
            for (unsigned int vertex = 0;
                 vertex < GeometryInfo<2>::vertices_per_face;
                 ++vertex)
              if (std::abs(cell->vertex(vertex).distance(centre_2d) -
                           hole_diameter / 2.0) < 1e-12)
                {
  ...
  template <int dim>
  void
  Solid<dim>::setup_system(LA::MPI::BlockVector &solution_delta)
  {
    ...
    pcout << "  Number of degrees of freedom per block: "
          << "[n_u, n_p, n_J] = [" << dofs_per_block[u_block] << ", "
          << dofs_per_block[p_block] << ", " << dofs_per_block[J_block] << "]"
          << std::endl;
    ...
    DoFTools::make_sparsity_pattern(
      dof_handler, bsp, constraints, false, this_mpi_process);
    bsp.compress();
  ...
  Solid<dim>::setup_qph()
  {
    ...
    FilteredIterator<typename DoFHandler<dim>::active_cell_iterator> cell(
      IteratorFilters::SubdomainEqualTo(this_mpi_process),
  ...
  template <int dim>
  void
  Solid<dim>::print_conv_footer(const LA::MPI::BlockVector &solution_delta)
  {
    pcout << std::string(132, '_') << std::endl;
    const std::pair<double, std::pair<double, double>> error_dil =
      get_error_dilation(get_solution_total(solution_delta));
    pcout << "Relative errors:" << std::endl
          ...
          << "v / V_0:\t" << error_dil.second.second << " / "
          << error_dil.second.first << " = "
    ...
    for (; cell != endc; ++cell)
      {
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
          {
            const double det_F_qp =
              determinant(Physics::Elasticity::Kinematics::F(
  ...
  template <int dim>
  void
  Solid<dim>::get_error_update(const LA::MPI::BlockVector &newton_update,
                               Errors &                    error_update)
  {
  ...
  template <int dim>
  LA::MPI::BlockVector
  Solid<dim>::get_solution_total(
    const LA::MPI::BlockVector &solution_delta) const
  {
  ...
  template <int dim>
  void
  Solid<dim>::assemble_system(const LA::MPI::BlockVector &solution_delta)
  {
    ...
    PerTaskData_ASM &                                     data) const
  {
    ...
    for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
      {
        const Tensor<2, dim> F = Physics::Elasticity::Kinematics::F(
    ...
    for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
      {
        ...
        {
          PointHistory<dim> *lqph_q_point_nc =
            const_cast<PointHistory<dim> *>(lqph[q_point].get());
                ...
                if ((i_group == u_block) && (j_group == u_block))
                ...
                else if ((i_group == u_block) && (j_group == p_block))
                  {
                    data.cell_matrix(i, j) +=
                      (symm_grad_Nx[i] * I) * Nx[j] * det_F * JxW;
                  }
                else if ((i_group == p_block) && (j_group == u_block))
                  {
                    data.cell_matrix(i, j) +=
                      Nx[i] * det_F * (symm_grad_Nx[j] * I) * JxW;
                  }
            ...
            cell->face(face)->boundary_id() == parameters.boundary_id_plus_Y)
          {
            ...
            for (unsigned int f_q_point = 0; f_q_point < n_q_points_f;
                 ++f_q_point)
              {
                ...
                static const double pressure_nom =
                  parameters.pressure / (parameters.scale * parameters.scale);
                const double time_ramp =
                  (time.current() < parameters.load_time ?
                     time.current() / parameters.load_time :
                     1.0);
    ...
    if (parameters.driver == "Dirichlet")
      {
        ...
        if (apply_dirichlet_bc == true)
          {
            if (time.current() <
                parameters.load_time + 0.01 * time.get_delta_t())
              {
                const double delta_length = parameters.length *
                                            (parameters.stretch - 1.0) *
                                            parameters.scale;
                const unsigned int n_stretch_steps =
                  parameters.load_time / time.get_delta_t();
  ...
  Solid<dim>::solve_linear_system(LA::MPI::BlockVector &newton_update)
  {
    ...
    const auto             K_uu =
      linear_operator<LA::MPI::Vector>(tangent_matrix.block(u_block, u_block));
    const auto K_up =
      linear_operator<LA::MPI::Vector>(tangent_matrix.block(u_block, p_block));
    const auto K_pu =
      linear_operator<LA::MPI::Vector>(tangent_matrix.block(p_block, u_block));
    const auto K_Jp =
      linear_operator<LA::MPI::Vector>(tangent_matrix.block(J_block, p_block));
    const auto K_JJ =
      linear_operator<LA::MPI::Vector>(tangent_matrix.block(J_block, J_block));
    ...
    ReductionControl solver_control_K_Jp_inv(
      tangent_matrix.block(J_block, p_block).m() *
    ...
    preconditioner_K_con_inv.initialize(
      ...
      LA::PreconditionAMG::AdditionalData(true /*elliptic*/,
                                          (parameters.poly_degree >
                                           1 /*higher_order_elements*/)));
  ...
  template <int dim>
  void
  Solid<dim>::output_results(const unsigned int timestep,
                             const double       current_time) const
  {
    ...
    struct Filename
    {
                       ...
                       const unsigned int n_digits = 4)
      {
        ...
        filename_vtu << "solution-" << (std::to_string(dim) + "d") << "."
      ...
      static std::string
      get_filename_pvtu(unsigned int timestep, const unsigned int n_digits = 4)
      {
        std::ostringstream filename_vtu;
        filename_vtu << "solution-" << (std::to_string(dim) + "d") << "."
      ...
      {
        std::ostringstream filename_vtu;
        filename_vtu << "solution-" << (std::to_string(dim) + "d") << ".pvd";
        return filename_vtu.str();
      }
    };
  ...
  template <int dim>
  void
  Solid<dim>::compute_vertex_positions(
    std::vector<double> &                 real_time,
    std::vector<std::vector<Point<dim>>> &tracked_vertices,
    const LA::MPI::BlockVector &          solution_total) const
  {
                ...
                if (cell->vertex(v).distance(pt_ref) < 1e-6 * parameters.scale)
                  {
                    for (unsigned int d = 0; d < dim; ++d)
                      vertex_update[p][d] =
                        solution_total(cell->vertex_dof_index(v, u_block + d));
    ...
    for (unsigned int p = 0; p < tracked_vertices.size(); ++p)
      {
        const int found_on_n_processes =
          Utilities::MPI::sum(int(vertex_found[p]), mpi_communicator);
        Assert(found_on_n_processes > 0,
               ExcMessage("Vertex not found on any processor"));
        Tensor<1, dim> update;
        for (unsigned int d = 0; d < dim; ++d)
          update[d] =
            Utilities::MPI::sum(vertex_update[p][d], mpi_communicator);
