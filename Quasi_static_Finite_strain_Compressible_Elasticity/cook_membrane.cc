...
namespace Cook_Membrane
{
  ...
  namespace Parameters
  {
    ...
    // Here we specify whether automatic differentiation is to be used to
    // assemble the linear system, and if so then what order of differentiation
    // is to be employed.
    ...
    void
    AssemblyMethod::declare_parameters(ParameterHandler &prm)
    {
    ...
    void
    AssemblyMethod::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Assembly method");
      {
  ...
  // As discussed in the literature and step-44, Neo-Hookean materials are a
  // type of hyperelastic materials.  The entire domain is assumed to be
  // composed of a compressible neo-Hookean material.  This class defines the
  // behaviour of this material within a one-field formulation.  Compressible
  // neo-Hookean materials can be described by a strain-energy function (SEF) $
  ...
  struct Assembler_Base
  {
    ...
    // as the shape-function values array (<code>Nx</code>) and a shape function
  ...
  try
    {
      ...
      if (parameters.automatic_differentiation_order == 0)
        ...
        {
          std::cout
            << "Assembly method: Residual computed manually; linearisation performed using AD."
            << std::endl;
