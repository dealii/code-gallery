...
private:
  ...
  std_cxx1x::shared_ptr<PETScWrappers::PreconditionBoomerAMG>
    preconditioner_Matrix_u;
  std_cxx1x::shared_ptr<PETScWrappers::PreconditionBoomerAMG>
    preconditioner_Matrix_v;
  std_cxx1x::shared_ptr<PETScWrappers::PreconditionBoomerAMG>
                                                              preconditioner_Matrix_w;
...
NavierStokesSolver<dim>::setup_VECTORS()
{
  ...
  SparsityTools::distribute_sparsity_pattern(
    ...
    mpi_communicator,
    locally_relevant_dofs_U);
  system_Matrix_u.reinit(mpi_communicator,
                         dsp_Matrix,
                         dof_handler_U.n_locally_owned_dofs_per_processor(),
                         dof_handler_U.n_locally_owned_dofs_per_processor(),
                         Utilities::MPI::this_mpi_process(mpi_communicator));
  system_Matrix_v.reinit(mpi_communicator,
                         dsp_Matrix,
                         dof_handler_U.n_locally_owned_dofs_per_processor(),
                         dof_handler_U.n_locally_owned_dofs_per_processor(),
                         Utilities::MPI::this_mpi_process(mpi_communicator));
  system_Matrix_w.reinit(mpi_communicator,
                         dsp_Matrix,
                         dof_handler_U.n_locally_owned_dofs_per_processor(),
                         dof_handler_U.n_locally_owned_dofs_per_processor(),
                         Utilities::MPI::this_mpi_process(mpi_communicator));
  ...
  SparsityTools::distribute_sparsity_pattern(
    ...
    mpi_communicator,
    locally_relevant_dofs_P);
  system_S.reinit(mpi_communicator,
                  dsp_S,
                  dof_handler_P.n_locally_owned_dofs_per_processor(),
                  dof_handler_P.n_locally_owned_dofs_per_processor(),
                  Utilities::MPI::this_mpi_process(mpi_communicator));
  ...
  SparsityTools::distribute_sparsity_pattern(
    ...
    mpi_communicator,
    locally_relevant_dofs_P);
  system_M.reinit(mpi_communicator,
                  dsp_M,
                  dof_handler_P.n_locally_owned_dofs_per_processor(),
                  dof_handler_P.n_locally_owned_dofs_per_processor(),
                  Utilities::MPI::this_mpi_process(mpi_communicator));
    ...
    if (cell_U->is_locally_owned())
      {
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
          {
            ...
            pressure_grad_u =
              (grad_pqn[q_point][0] + 4. / 3 * grad_psiqn[q_point][0] -
               1. / 3 * grad_psiqnm1[q_point][0]);
            pressure_grad_v =
              (grad_pqn[q_point][1] + 4. / 3 * grad_psiqn[q_point][1] -
               1. / 3 * grad_psiqnm1[q_point][1]);
            if (dim == 3)
              pressure_grad_w =
                (grad_pqn[q_point][2] + 4. / 3 * grad_psiqn[q_point][2] -
            ...
            if (LEVEL_SET == 1) // use level set to define rho and nu
              ...
              {
                rho_value =
                  rho_function.value(fe_values_U.quadrature_point(q_point));
                nu_value =
                  nu_function.value(fe_values_U.quadrature_point(q_point));
              }
            ...
            rho_star = rho_value; // This is because we consider rho*u_t instead
                                  // of (rho*u)_t
            rho = rho_value;

            // FORCE TERMS
            force_function.vector_value(fe_values_U.quadrature_point(q_point),
                                        force_terms);
                  ...
                  for (unsigned int j = 0; j < dofs_per_cell; ++j)
                    {
                      if (dim == 2)
                        cell_A_u(i, j) +=
                          (rho_star * shape_value[i] * shape_value[j] +
        ...
        // distribute
        if (rebuild_Matrix_U == true)
          constraints.distribute_local_to_global(cell_A_u,
                                                 local_dof_indices,
                                                 system_Matrix_u);
        constraints.distribute_local_to_global(cell_rhs_u,
                                               local_dof_indices,
                                               system_rhs_u);
        constraints.distribute_local_to_global(cell_rhs_v,
                                               local_dof_indices,
                                               system_rhs_v);
        if (dim == 3)
      ...
      if (rebuild_Matrix_U_preconditioners)
        {
          ...
          preconditioner_Matrix_u.reset(
            new PETScWrappers::PreconditionBoomerAMG(
              system_Matrix_u,
              PETScWrappers::PreconditionBoomerAMG::AdditionalData(false)));
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
          {
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
              {
                cell_rhs_psi(i) +=
                  -3. / 2. / time_step * rho_min * divU * shape_value[i] * JxW;
                ...
                if (rebuild_S_M == true)
                  {
                    for (unsigned int j = 0; j < dofs_per_cell; ++j)
                      if (i == j)
                        {
                          cell_S(i, j) +=
                            shape_grad[i] * shape_grad[j] * JxW + 1E-10;
  ...
  const PETScWrappers::MPI::Vector &rhs)
{
  ...
  if (solver_control.last_step() > MAX_NUM_ITER_TO_RECOMPUTE_PRECONDITIONER)
    rebuild_Matrix_U_preconditioners = true;
  if (verbose == true)
    pcout << "   Solved U in " << solver_control.last_step() << " iterations."
          << std::endl;
}
  ...
  const PETScWrappers::MPI::Vector &rhs)
{
  ...
  if (solver_control.last_step() > MAX_NUM_ITER_TO_RECOMPUTE_PRECONDITIONER)
    rebuild_S_M_preconditioners = true;
  if (verbose == true)
    pcout << "   Solved P in " << solver_control.last_step() << " iterations."
          << std::endl;
}
