...
MultiPhase<dim>::get_boundary_values_U()
{
  ...
  std::map<types::global_dof_index, double>::const_iterator boundary_value_u =
    map_boundary_values_u.begin();
  std::map<types::global_dof_index, double>::const_iterator boundary_value_v =
        ...
        if (cell_U->face(face)->at_boundary())
          {
            ...
            if (fe_face_values.normal_vector(0) * u < -1e-14)
              cell_U->face(face)->set_boundary_id(
                10); // SET ID 10 to inlet BOUNDARY (10 is an arbitrary number)
          }
}
  ...
  std::vector<double> &                 boundary_values_phi)
{
  ...
  std::map<types::global_dof_index, double>::const_iterator boundary_value_phi =
...
MultiPhase<dim>::output_rho()
{
  ...
  const std::string filename =
    ("sol_rho-" + Utilities::int_to_string(output_number, 3) + "." +
     Utilities::int_to_string(triangulation.locally_owned_subdomain(), 4));
  std::ofstream output((filename + ".vtu").c_str());
  data_out.write_vtu(output);
...
MultiPhase<dim>::run()
{
  ...
  // TRANSPORT SOLVER
  LevelSetSolver<dim> transport_solver(degree_LS,
                                       degree_U,
                                       time_step,
                                       cK,
                                       cE,
