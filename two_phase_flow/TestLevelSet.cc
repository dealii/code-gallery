...
{
public:
  TestLevelSet(const unsigned int degree_LS, const unsigned int degree_U);
  ~TestLevelSet();
  void
  run();
...
private:
  ...
  // POST PROCESSING //
  void
  process_solution(parallel::distributed::Triangulation<dim> &triangulation,
...
template <int dim>
void
TestLevelSet<dim>::get_interpolated_velocity()
{
  // velocity in x
...
template <int dim>
void
TestLevelSet<dim>::set_boundary_inlet()
{
  const QGauss<dim - 1> face_quadrature_formula(1); // center of the face
  FEFaceValues<dim>     fe_face_values(fe_U,
        ...
        if (cell_U->face(face)->at_boundary())
          {
            ...
            fe_face_values.get_function_values(locally_relevant_solution_v,
                                               v_value);
            if (dim == 3)
              fe_face_values.get_function_values(locally_relevant_solution_w,
                                                 w_value);
            u[0] = u_value[0];
            u[1] = v_value[0];
            if (dim == 3)
...
template <int dim>
void
TestLevelSet<dim>::get_boundary_values_phi(
  std::vector<unsigned int> &boundary_values_id_phi,
  std::vector<double> &      boundary_values_phi)
{
...
TestLevelSet<dim>::setup()
{
  ...
  locally_owned_dofs_U_disp_field =
    dof_handler_U_disp_field.locally_owned_dofs();
  ...
  completely_distributed_solution_phi.reinit(
    mpi_communicator,
  ...
  completely_distributed_solution_u.reinit(
    mpi_communicator,
  ...
  completely_distributed_solution_v.reinit(
    mpi_communicator,
    dof_handler_U.n_dofs(),
    dof_handler_U.n_locally_owned_dofs());
  // init vectors for w
  ...
  completely_distributed_solution_w.reinit(
    mpi_communicator,
    dof_handler_U.n_dofs(),
    dof_handler_U.n_locally_owned_dofs());
  init_constraints();
...
template <int dim>
void
TestLevelSet<dim>::initial_condition()
{
  time = 0;
  // Initial conditions //
...
template <int dim>
void
TestLevelSet<dim>::init_constraints()
{
  constraints.clear();
  ...
  DoFTools::make_hanging_node_constraints(dof_handler_LS,
                                          constraints_disp_field);
...
template <int dim>
void
TestLevelSet<dim>::process_solution(
  parallel::distributed::Triangulation<dim> &triangulation,
  DoFHandler<dim> &                          dof_handler_LS,
  PETScWrappers::MPI::Vector &               solution)
{
  ...
  u_L1_error =
    std::sqrt(Utilities::MPI::sum(u_L1_error * u_L1_error, mpi_communicator));
...
template <int dim>
void
TestLevelSet<dim>::output_results()
{
  output_solution();
  output_number++;
}
...
template <int dim>
void
TestLevelSet<dim>::output_solution()
{
...
TestLevelSet<dim>::run()
{
  ...
  // GridGenerator::hyper_rectangle(triangulation, Point<dim>(0.0,0.0),
  // Point<dim>(1.0,1.0), true);
  ...
  // for Reconstruction of MATERIAL FIELDS
  min_h =
    GridTools::minimal_cell_diameter(triangulation) / std::sqrt(dim) / degree;
  eps       = 1 * min_h; // For reconstruction of density in Navier Stokes
  sharpness = sharpness_integer *
              min_h; // adjust value of sharpness (for init cond of phi)
  rho_fluid = 1000;
  rho_air   = 1;
  ...
  level_set.set_boundary_conditions(boundary_values_id_phi,
                                    boundary_values_phi);
      ...
      if (VARIABLE_VELOCITY)
        {
          ...
          level_set.set_velocity(locally_relevant_solution_u,
                                 locally_relevant_solution_v);
        }
      ////////////////////////////
