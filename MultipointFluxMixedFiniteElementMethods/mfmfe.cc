...
namespace MFMFE
{
  ...
  namespace DataStructures
  {
    ...
    // full matrices. For this, a data structure with fast lookup is crucial,
    // hence the hash table, with the keys as Point<dim>
    template <int dim>
    struct hash_points
    {
      size_t
    ...
    // Here, the actual hash-tables are defined. We use the C++ STL
    // <code>unordered_map</code>, with the hash function specified above. For
    // convenience these are aliased as follows
    template <int dim>
    using PointToMatrixMap = std::unordered_map<
      Point<dim>,
    ...
    template <int dim>
    NodeAssemblyScratchData<dim>::NodeAssemblyScratchData(
    ...
    VertexEliminationScratchData::VertexEliminationScratchData(
      const VertexEliminationScratchData &scratch_data)
  ...
  // The main class, besides the constructor and destructor, has only one public
  // member <code>run()</code>, similarly to the tutorial programs. The private
  // members can be grouped into the ones that are used for the cell-wise
  // assembly, vertex elimination, pressure solve, vertex velocity recovery and
  // postprocessing. Apart from the MFMFE-specific data structures, the rest of
  // the members should look familiar.
  template <int dim>
  ...
  private:
    ...
    void
    copy_cell_to_node(
      const DataStructures::NodeAssemblyCopyData<dim> &copy_data);
    void
    node_assembly();
    void
    ...
    void
    copy_node_to_system(
      const DataStructures::NodeEliminationCopyData<dim> &copy_data);
    void
    pressure_assembly();
    void
    solve_pressure();
    ...
    std::unordered_map<Point<dim>,
                       FullMatrix<double>,
                       DataStructures::hash_points<dim>>
      pressure_matrix;
    std::unordered_map<Point<dim>,
                       FullMatrix<double>,
                       DataStructures::hash_points<dim>>
  ...
  // and then construct the vector valued element belonging to the space $V_h^k$
  // described in the introduction. The constructor also takes care of
  // initializing the computing timer, as it is of interest for us how well our
  // method performs.
  template <int dim>
  MultipointMixedDarcyProblem<dim>::MultipointMixedDarcyProblem(
    const unsigned int degree)
    : degree(degree)
    , fe(FE_RT_Bubbles<dim>(degree), 1, FE_DGQ<dim>(degree - 1), 1)
  ...
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::reset_data_structures()
  {
  ...
  // obtained from local cell integration into the correct place in a
  // matrix/vector corresponging to a specific node.
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::copy_cell_to_node(
    const DataStructures::NodeAssemblyCopyData<dim> &copy_data)
  {
  ...
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::assemble_system_cell(
    const typename DoFHandler<dim>::active_cell_iterator &cell,
    DataStructures::NodeAssemblyScratchData<dim> &        scratch_data,
    DataStructures::NodeAssemblyCopyData<dim> &           copy_data)
  {
    ...
    k_inverse.value_list(scratch_data.fe_values.get_quadrature_points(),
                         scratch_data.k_inverse_values);
    rhs.value_list(scratch_data.fe_values.get_quadrature_points(),
    ...
    // One, we need to be able to assemble the communication between velocity
    // and pressure variables and put it on the right place in our final, local
    // version of the B matrix. This is a little messy, as such communication is
    // not in fact local, so we do it in two steps. First, we compute all
    // relevant LHS and RHS
    for (unsigned int q = 0; q < n_q_points; ++q)
      {
        ...
        for (unsigned int k = 0; k < dofs_per_cell; ++k)
          {
            scratch_data.phi_u[k] =
              scratch_data.fe_values[velocity].value(k, q);
            scratch_data.div_phi_u[k] =
              scratch_data.fe_values[velocity].divergence(k, q);
            scratch_data.phi_p[k] =
              scratch_data.fe_values[pressure].value(k, q);
          }
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i)
          {
            for (unsigned int j = n_vel; j < dofs_per_cell; ++j)
              {
                double div_term =
                  (-scratch_data.div_phi_u[i] * scratch_data.phi_p[j] -
                   scratch_data.phi_p[i] * scratch_data.div_phi_u[j]) *
            ...
            double source_term = -scratch_data.phi_p[i] *
                                 scratch_data.rhs_values[q] *
                                 scratch_data.fe_values.JxW(q);

            if (std::abs(scratch_data.phi_p[i]) > 1.e-12 ||
    ...
    // Then, by making another pass, we compute the mass matrix terms and
    // incorporate the divergence form and RHS accordingly. This second pass,
    // allows us to know where the total contribution will be put in the nodal
    // data structures, as with this choice of quadrature rule and finite
    // element only the basis functions corresponding to the same quadrature
    // points yield non-zero contribution.
    for (unsigned int q = 0; q < n_q_points; ++q)
      {
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i)
          for (unsigned int j = i; j < dofs_per_cell; ++j)
            {
              double mass_term =
                scratch_data.phi_u[i] * scratch_data.k_inverse_values[q] *
                scratch_data.phi_u[j] * scratch_data.fe_values.JxW(q);

              if (std::abs(mass_term) > 1.e-12)
                {
                  copy_data.cell_mat[p][std::make_pair(
                    copy_data.local_dof_indices[i],
                    copy_data.local_dof_indices[j])] += mass_term;
                  vel_indices.insert(i);
                  copy_data.local_vel_indices[p].insert(
            ...
            if (std::abs(el.second) > 1.e-12)
              {
                ...
                copy_data.local_pres_indices[p].insert(
                  copy_data.local_dof_indices[el.first]);
              }
      }
      ...
      if (cell->at_boundary(face_no))
        {
          ...
          for (unsigned int q = 0; q < n_face_q_points; ++q)
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
              {
                double tmp =
                  -(scratch_data.fe_face_values[velocity].value(i, q) *
  ...
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::node_assembly()
  {
    ...
    WorkStream::run(dof_handler.begin_active(),
                    ...
                    *this,
                    ...
                    DataStructures::NodeAssemblyScratchData<dim>(
                      fe, triangulation, quad, face_quad),
  ...
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::make_cell_centered_sp()
  {
    ...
    for (auto el : node_matrix)
      for (pi_it = pressure_indices[el.first].begin(), i = 0;
           ...
           ++pi_it, ++i)
        for (pj_it = pi_it, j = 0; pj_it != pressure_indices[el.first].end();
             ++pj_it, ++j)
  ...
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::nodal_elimination(
    const typename DataStructures::PointToMatrixMap<dim>::iterator &n_it,
    DataStructures::VertexEliminationScratchData &scratch_data,
    DataStructures::NodeEliminationCopyData<dim> &copy_data)
  {
      ...
      for (vi_it = velocity_indices.at((*n_it).first).begin(), i = 0;
           ...
           ++vi_it, ++i)
        {
          ...
          for (vj_it = velocity_indices.at((*n_it).first).begin(), j = 0;
               ...
               ++vj_it, ++j)
            {
              scratch_data.velocity_matrix.add(
                i,
                j,
                node_matrix[(*n_it).first][std::make_pair(*vi_it, *vj_it)]);
              if (j != i)
                scratch_data.velocity_matrix.add(
                  j,
                  i,
          ...
          for (p_it = pressure_indices.at((*n_it).first).begin(), j = 0;
               ...
               ++p_it, ++j)
            copy_data.pressure_matrix.add(
              i, j, node_matrix[(*n_it).first][std::make_pair(*vi_it, *p_it)]);
    ...
    copy_data.Ainverse.vmult(scratch_data.tmp_rhs1,
                             copy_data.velocity_rhs,
                             false);
    copy_data.pressure_matrix.Tvmult(scratch_data.tmp_rhs3,
                                     scratch_data.tmp_rhs1,
                                     false);
  ...
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::copy_node_to_system(
    const DataStructures::NodeEliminationCopyData<dim> &copy_data)
  {
      ...
      for (pi_it = pressure_indices[copy_data.p].begin(), i = 0;
           ...
           ++pi_it, ++i)
        {
          ...
          for (pj_it = pressure_indices[copy_data.p].begin(), j = 0;
               ...
               ++pj_it, ++j)
            pres_system_matrix.add(*pi_it - n_v,
                                   *pj_it - n_v,
                                   copy_data.node_pres_matrix(i, j));
  ...
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::pressure_assembly()
  {
  ...
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::velocity_assembly(
    const typename DataStructures::PointToMatrixMap<dim>::iterator &n_it,
    DataStructures::VertexEliminationScratchData &scratch_data,
    DataStructures::NodeEliminationCopyData<dim> &copy_data)
  {
    ...
    pressure_matrix[(*n_it).first].vmult(scratch_data.tmp_rhs2,
                                         scratch_data.local_pressure_solution,
                                         false);
    scratch_data.tmp_rhs2 *= -1.0;
    scratch_data.tmp_rhs2 += velocity_rhs[(*n_it).first];
    A_inverse[(*n_it).first].vmult(copy_data.vertex_vel_solution,
                                   scratch_data.tmp_rhs2,
                                   false);
  ...
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::copy_node_velocity_to_global(
    const DataStructures::NodeEliminationCopyData<dim> &copy_data)
  {
  ...
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::velocity_recovery()
  {
  ...
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::solve_pressure()
  {
  ...
  // velocity error, one must provide gradients in the
  // <code>ExactSolution</code> class implementation to avoid exceptions. The
  // only noteworthy thing here is that we again use lower order quadrature rule
  // instead of projecting the solution to an appropriate space in order to show
  // superconvergence, which is mathematically justified.
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::compute_errors(const unsigned cycle)
  {
  ...
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::output_results(const unsigned int cycle,
                                                   const unsigned int refine)
  {
    ...
    convergence_table.set_tex_caption("Velocity,L2",
                                      "$ \\|\\u - \\u_h\\|_{L^2} $");
    convergence_table.set_tex_caption(
      "Velocity,Hdiv", "$ \\|\\nabla\\cdot(\\u - \\u_h)\\|_{L^2} $");
    convergence_table.set_tex_caption("Pressure,L2", "$ \\|p - p_h\\|_{L^2} $");
    convergence_table.set_tex_caption("Pressure,L2-nodal",
                                      "$ \\|Qp - p_h\\|_{L^2} $");
    convergence_table.set_tex_format("cells", "r");
    convergence_table.set_tex_format("dofs", "r");

    convergence_table.evaluate_convergence_rates(
      "Velocity,L2", ConvergenceTable::reduction_rate_log2);
    convergence_table.evaluate_convergence_rates(
      "Velocity,Hdiv", ConvergenceTable::reduction_rate_log2);
    convergence_table.evaluate_convergence_rates(
      "Pressure,L2", ConvergenceTable::reduction_rate_log2);
    convergence_table.evaluate_convergence_rates(
      "Pressure,L2-nodal", ConvergenceTable::reduction_rate_log2);
  ...
  template <int dim>
  void
  MultipointMixedDarcyProblem<dim>::run(const unsigned int refine)
  {
