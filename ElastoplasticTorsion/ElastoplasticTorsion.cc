...
namespace nsp
{
  ...
  ParameterReader::declare_parameters()
  {
    prm.enter_subsection("Global Parameters");
    {
      ...
      prm.declare_entry("known_solution",
                        "true",
                        Patterns::Bool(),
    ...
    prm.enter_subsection("Mesh & Refinement Parameters");
    {
      ...
      prm.declare_entry("No of adaptive refinements",
                        "8",
                        Patterns::Integer(0),
                        "Number of global adaptive mesh refinements");
      prm.declare_entry("top_fraction_of_cells",
                        "0.25",
                        Patterns::Double(0),
                        "refinement threshold");
      prm.declare_entry("bottom_fraction_of_cells",
                        "0.05",
                        Patterns::Double(0),
    ...
    {
      prm.declare_entry("Descent_direction",
                        "0",
                        Patterns::Integer(0, 1),
                        "0: Preconditioned descent, 1: Newton Method");
      prm.declare_entry("init_p", "10", Patterns::Double(2), "Initial p");
      prm.declare_entry("delta_p", "50", Patterns::Double(0), "increase of p");
      prm.declare_entry("Max_CG_it",
                        "1500",
                        Patterns::Integer(1),
                        "Maximum Number of CG iterations");
      prm.declare_entry("CG_tol",
                        "1e-10",
                        Patterns::Double(0),
                        "Tolerance for CG iterations");
      prm.declare_entry("max_LS_it",
                        "45",
                        Patterns::Integer(1),
      ...
      prm.declare_entry("init_step_length",
                        "1e-2",
                        Patterns::Double(0),
                        "initial step length in line-search");
      prm.declare_entry("Max_inner",
                        "800",
                        Patterns::Integer(1),
  ...
  // ******************************************************************************************//
  // The solution of the elastoplastic torsion problem on the unit disk with
  // rhs=4.
  ...
  public:
    ...
    {}
    virtual double
    value(const Point<dim> &pto, const unsigned int component = 0) const;
    virtual Tensor<1, dim>
    gradient(const Point<dim> &pto, const unsigned int component = 0) const;
  };

  template <int dim>
  double
  Solution<dim>::value(const Point<dim> &pto, const unsigned int) const
  {
  ...
  template <int dim>
  Tensor<1, dim>
  Solution<dim>::gradient(const Point<dim> &pto, const unsigned int) const
  {
    double r = sqrt(pto.square());
    if (r < 0.5)
      return -2.0 * pto;
  ...
  template <int dim>
  ComputeMultiplier<dim>::ComputeMultiplier(double pe)
    : p(pe)
  {}
  ...
  template <int dim>
  void
  ComputeMultiplier<dim>::compute_derived_quantities_scalar(
  ...
  private:
    ...
    set_boundary_values();
    double
    phi(const double alpha) const;
    bool
    checkWolfe(double &alpha, double &phi_alpha) const;
    bool
    determine_step_length(const int inner_it);
    void
    print_it_message(const int counter, bool ks);
    void
    output_results(unsigned int refinement) const;
    void
    format_convergence_tables();
    void
    process_solution(const unsigned int cycle);
    void
    process_multiplier(const unsigned int cycle, const int iter, double time);
    double
    dual_error() const;
    double
    dual_infty_error() const;
    double
    W(double Du2) const;
    double
    Wp(double Du2) const;
  ...
  template <int dim>
  double
  BoundaryValues<dim>::value(const Point<dim> & /*pto*/,
  ...
  template <int dim>
  void
  ElastoplasticTorsion<dim>::print_it_message(const int counter, bool ks)
  {
    if (ks)
      {
        process_solution(counter);
        std::cout << "iteration=" << counter + 1 << "  J(u_h)= " << phi_zero
                  << ", H1 error: " << H1_error
      ...
      {
        std::cout << "iteration= " << counter + 1 << " J(u_h)= " << phi_alpha
                  << " J'(u_h)= " << phip << std::endl;
      }
  }
  ...
  template <int dim>
  void
  ElastoplasticTorsion<dim>::format_convergence_tables()
  {
  ...
  template <int dim>
  void
  ElastoplasticTorsion<dim>::process_solution(const unsigned int it)
  {
  ...
  template <int dim>
  void
  ElastoplasticTorsion<dim>::process_multiplier(const unsigned int cycle,
                                                const int          iter,
                                                double             time)
  {
  ...
  template <int dim>
  void
  ElastoplasticTorsion<dim>::setup_system(const bool initial_step)
  {
    if (initial_step)
      {
  ...
  template <int dim>
  double
  ElastoplasticTorsion<dim>::W(double Du2) const
  {
    return Du2;
  }
  ...
  template <int dim>
  double
  ElastoplasticTorsion<dim>::Wp(double /*Du2*/) const
  {
    return 1.0;
  }
  ...
  template <int dim>
  double
  ElastoplasticTorsion<dim>::G(double /*Du2*/) const
  {
    return 1.0;
  }
  /***************************************************************************************/
  ...
  template <int dim>
  void
  ElastoplasticTorsion<dim>::assemble_system()
  {
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
          {
            ...
            long double a =
              old_solution_gradients[q_point] * old_solution_gradients[q_point];
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
              {
                ...
                cell_rhs(i) -=
                  (fe_values.shape_grad(i, q_point) *
                     old_solution_gradients[q_point] * (Wp(a) + coeff) -
  ...
  template <int dim>
  void
  ElastoplasticTorsion<dim>::refine_mesh()
  {
  ...
  /*******************************************************************************************/
  // Dump the norm of the gradient and the lagrange multiplier in vtu format for
  // visualization
  template <int dim>
  void
  ElastoplasticTorsion<dim>::output_results(unsigned int counter) const
  {
  ...
  template <int dim>
  void
  ElastoplasticTorsion<dim>::set_boundary_values()
  {
    std::map<types::global_dof_index, double> boundary_values;
    VectorTools::interpolate_boundary_values(dof_handler,
                                             0,
                                             BoundaryValues<dim>(),
                                             boundary_values);
    for (std::map<types::global_dof_index, double>::const_iterator bp =
           boundary_values.begin();
         bp != boundary_values.end();
         ++bp)
  ...
  template <int dim>
  double
  ElastoplasticTorsion<dim>::phi(const double alpha) const
  {
    double                   obj = 0.0;
    const RightHandSide<dim> right_hand_side;
  ...
  template <int dim>
  double
  ElastoplasticTorsion<dim>::dual_error() const
  {
    double obj = 0.0;
  ...
  template <int dim>
  double
  ElastoplasticTorsion<dim>::dual_infty_error() const
  {
    double               obj = 0.0;
    const QTrapez<1>     q_trapez;
  ...
  template <int dim>
  bool
  ElastoplasticTorsion<dim>::checkWolfe(double &alpha, double &phi_alpha) const
  {
  ...
  template <int dim>
  bool
  ElastoplasticTorsion<dim>::determine_step_length(const int inner_it)
  {
    unsigned int it = 0;
    bool         done;
    double       alpha, nalpha;
    ...
    while ((!done) & (it < max_LS_it))
      {
        ...
        nalpha =
          -(phip * alpha * alpha) / (2 * (phi_alpha - phi_zero - phip * alpha));
        ...
        if (done)
          std::cerr << ", finished with steplength= " << alpha
                    << ", fcn value= " << phi_alpha << std::endl;
        it = it + 1;
      }
    if (!done)
      {
        std::cerr << ", max. no. of iterations reached wiht steplength= "
                  << alpha << ", fcn value= " << phi_alpha << std::endl;
        return false;
      }
    else
      {
  ...
  template <int dim>
  void
  ElastoplasticTorsion<dim>::init_mesh()
  {
    // get parameters
    prm.enter_subsection("Mesh & Refinement Parameters");
    ...
    if (domain_id == 0)
    ...
    else if (domain_id == 2)
      {
        ...
        */
        ...
        GridGenerator::hyper_rectangle(tria1,
                                       Point<2>(0.0, 0.0),
                                       Point<2>(0.4, 0.8));
        GridGenerator::hyper_rectangle(tria2,
                                       Point<2>(0.0, 0.8),
                                       Point<2>(0.4, 1.0));
        GridGenerator::hyper_rectangle(tria3,
                                       Point<2>(0.4, 0.0),
                                       Point<2>(0.6, 0.8));
        GridGenerator::hyper_rectangle(tria4,
                                       Point<2>(0.6, 0.0),
                                       Point<2>(1.0, 0.8));
        GridGenerator::hyper_rectangle(tria5,
                                       Point<2>(0.6, 0.8),
                                       Point<2>(1.0, 1.0));
  ...
  template <int dim>
  bool
  ElastoplasticTorsion<dim>::solve(const int inner_it)
  {
    prm.enter_subsection("Algorithm Parameters");
  ...
  ElastoplasticTorsion<dim>::run()
  {
    ...
    int refinement =
      0; //  Refinements  performed (adaptive) = outer iterations 2nd loop
    ...
    while (p < actual_p) // outer iteration, increasing p.
      {
        std::cout << "--Preparing initial condition with p=" << p
                  << " iter.= " << global_it << "  .-- " << std::endl;
        timer.restart();
        for (int inner_iteration = 0; inner_iteration < max_inner;
             ++inner_iteration, ++global_it)
          {
            assemble_system();
            well_solved = solve(inner_iteration);
            print_it_message(global_it, known_solution);
            if (((system_rhs.l2_norm() / std::sqrt(system_rhs.size()) < 1e-4) &
                 (cycle < 1)) |
                ((system_rhs.l2_norm() / std::sqrt(system_rhs.size()) < 1e-5) &
                 (cycle >= 1)) |
                !well_solved)
              break;
          }
    ...
    std::cout << "============ Solving  problem with p=" << p
              << " ==================" << std::endl;
    ...
    while ((cycle < adapt_ref) & well_solved)
      {
        ...
        for (int inner_iteration = 0; inner_iteration < max_inner;
             ++inner_iteration, ++global_it)
          {
            ...
            if (((system_rhs.l2_norm() / std::sqrt(system_rhs.size()) < eps) &
                 (refinement < hi_th)) |
                ((system_rhs.l2_norm() / std::sqrt(system_rhs.size()) <
                  hi_eps) |
                 (!well_solved)))
              break;
          }
        ...
        if (known_solution)
          {
            process_multiplier(cycle, global_it, ptime);
            std::cout << "finished with H1 error: " << H1_error
                      << ", dual error (L1): " << dual_L1_error
                      << "dual error (L infty): " << dual_L_infty_error
                      << std::endl;
          }
    ...
    if (known_solution)
      {
        ...
        std::string   dual_error_filename =
          "dual_error" + Method + elements + ".tex";
